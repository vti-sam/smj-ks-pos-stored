...
- `TabletHost.StartHost()` khởi động transport qua `DeviceHostTransport`; Named Pipe là đường chính cho application mới, WindowMessage là compatibility path cho client legacy.
...
- Không expose Host concrete class cho `TabetPos.DeviceCtrl`; DeviceCtrl chỉ nên giao tiếp bằng Named Pipe.
