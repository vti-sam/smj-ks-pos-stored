...
`sources/TabetPosBoilerplate` liên kết với tầng Host bằng Named Pipe.
...
Áp dụng cho basic design của tầng Application trong `TabetPosBoilerplate`.
...
`TabetPosBoilerplate` đã có thiết kế Host mới rõ hơn, dùng 3 Named Pipe:

- `TabetPos.Host.Command`: Application gửi command và nhận response đồng bộ.
- `TabetPos.Host.Event`: Host publish event thiết bị bất đồng bộ về Application.
- `TabetPos.Host.Stream`: Host publish dữ liệu stream, ví dụ scanner.
...
- `DeviceCommandRouter`
- `TabletHost.ProcessNamedPipeCommand(...)`
...
sequenceDiagram
    ...
    Pipe->>Host: Gửi JSON vào TabetPos.Host.Command
...
sequenceDiagram
    participant Device as "Device / OPOS"
    participant Host as "タブレットPOS Host"
    participant EventPipe as "TabetPos.Host.Event"
...
sequenceDiagram
    ...
    participant Host as "タブレットPOS Host"
    participant StreamPipe as "TabetPos.Host.Stream"
...
|---|---|
| Không kết nối được `TabetPos.Host.Command` | Báo Host chưa khởi động hoặc không thể kết nối dịch vụ thiết bị. |
...
Hiện `TabetPos.Applications` chưa có Named Pipe Client hoặc device gateway tương ứng. Cần bổ sung ở mức Application/Infrastructure:
...
- Background listener cho `TabetPos.Host.Event`.
- Background listener cho `TabetPos.Host.Stream`.
- DI registration trong `TabetPos.Applications/Composition/DependencyInjection.cs`.

ViewModel không gọi trực tiếp implementation Named Pipe; ViewModel chỉ gọi Application Service hoặc port nghiệp vụ.

## Ghi chú thiết kế

- Với command cần kết quả ngay, dùng `TabetPos.Host.Command`.
- Với callback thiết bị thông thường, dùng `TabetPos.Host.Event`.
- Với dữ liệu liên tục hoặc tần suất cao, dùng `TabetPos.Host.Stream`.
...
- `sources/TabetPosBoilerplate/TabetPos.Host/src/TabletHost/DeviceHost/NamedPipeContracts.cs`
- `sources/TabetPosBoilerplate/TabetPos.Host/src/TabletHost/DeviceHost/NamedPipeCommandServer.cs`
- `sources/TabetPosBoilerplate/TabetPos.Host/src/TabletHost/DeviceHost/DeviceCommandRouter.cs`
- `sources/TabetPosBoilerplate/TabetPos.Host/src/TabletHost/DeviceHost/TabletHost.NamedPipe.cs`
- `sources/TabetPosBoilerplate/TabetPos.Host/src/TabletHost/DeviceHost/TabletHost.cs`
