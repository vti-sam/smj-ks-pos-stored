...
# Kiến trúc base TabetPos.Applications - 2026-05-21

## Mục tiêu

`TabetPos.Applications` đã được refactor sang kiến trúc tối giản để phát triển tiếp app POS lớn, dùng Command/Query Handler thay cho folder `UseCases` chung chung.

Không đụng `TabetPos.Host`; scope chỉ nằm trong `sources/TabetPosBoilerplate/TabetPos.Applications`.
...
```powershell
rtk dotnet build TabetPos.Applications.csproj -v:m
rtk dotnet build TabetPos.Applications.slnx -v:m
```
...
- `TabetPos.Applications.csproj` và `TabetPos.ApplicationControls.csproj` dùng target framework theo platform, giống pattern của `MauiPOSX.Application`.
- Trên Windows dùng:
