...
# Port Application base cho TabetPosBoilerplate - 2026-05-21

## Bối cảnh

- Nguồn base: `sources/MauiPOSX/MauiPOSX.Application`.
- Đích: `sources/TabetPosBoilerplate/TabetPos.Applications`.
- Scope user đã confirm: chỉ lấy base code ở tầng Application. `TabetPos.Host` là phần riêng, không copy.
...
- Sửa project reference hardcoded trong `TabetPos.Applications` và `TabetPos.ApplicationControls` sang relative path trong boilerplate.
- Sửa binding sample XAML có sẵn từ `InnerReturnCommand` sang `ReturnCommand`.

## Verify

- Command: `rtk dotnet build TabetPos.Applications.csproj -f net10.0-windows10.0.19041.0 -v:m`
