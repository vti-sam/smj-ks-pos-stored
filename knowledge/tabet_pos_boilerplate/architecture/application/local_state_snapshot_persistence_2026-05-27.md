...
`TabetPos.Applications` thêm nền tảng lưu ViewModel snapshot bằng SQLite để giữ state cục bộ của terminal app sau chuyển màn hình, app kill/crash hoặc mất điện.
...
```powershell
rtk dotnet build TabetPos.Applications.csproj -f net10.0-windows10.0.19041.0 -v:m
```
