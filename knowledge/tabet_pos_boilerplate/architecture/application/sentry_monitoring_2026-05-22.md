...
# Sentry monitoring base cho TabetPos.Applications

## Bối cảnh

- Ngày ghi: 2026-05-22.
- Scope: `sources/TabetPosBoilerplate/TabetPos.Applications`.
...
- `TabetPos.Applications/Infrastructure/Monitoring/SentrySettings.cs`
- `TabetPos.Applications/Infrastructure/Monitoring/SensitiveDataMasker.cs`
- `TabetPos.Applications/Infrastructure/Monitoring/SentryConfiguration.cs`
- `TabetPos.Applications/Infrastructure/Monitoring/SentryMonitoringService.cs`
- `TabetPos.Applications/Ports/Monitoring/IMonitoringService.cs`
- `TabetPos.Applications/Ports/Monitoring/MonitoringEvent.cs`
- `TabetPos.Applications/Ports/Monitoring/MonitoringContext.cs`
- `TabetPos.Applications/Ports/Monitoring/MonitoringLevel.cs`
- `TabetPos.Applications/MauiProgram.cs` wire `UseSentry(...)`.
- `TabetPos.Applications/Presentation/Views/MainPage.xaml` là màn test `結合テスト` tạm thời.
- `TabetPos.Applications/Presentation/ViewModels/MainPageViewModel.cs` có command gửi Sentry smoke test và masking test.
- `TabetPos.Applications.csproj` thêm package `Sentry.Maui` version `6.5.0`.
...
- Khi NuGet bị lock ở `%LOCALAPPDATA%\Temp\NuGetScratch`, có thể build bằng temp riêng:
  - `rtk cmd /c "set TEMP=C:\tmp&& set TMP=C:\tmp&& dotnet build TabetPos.Applications.csproj -v:m"`
...
- Chưa có test project phù hợp trong `sources/TabetPosBoilerplate` tại thời điểm ghi memory.
