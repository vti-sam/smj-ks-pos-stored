...
- `TabetPos.Host` là host DeviceServer được di trú để điều khiển ngoại vi POS.
- Đường dẫn runtime chính: `AppServer` -> `TabletHost` -> `TabletDeviceManager` -> triển khai thiết bị.
- Các lệnh bên ngoài có thể đến từ WindowMessage cũ hoặc Named Pipe mới.
- Các triển khai thiết bị được tải từ `host_device_config.json` thông qua `TabletDeviceSettingBase` và factory đã đăng ký trong `HostDeviceClassRegistry`.
