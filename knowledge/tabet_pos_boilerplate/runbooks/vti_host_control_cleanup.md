...
source:
  ...
  - sources/TabetPosBoilerplate/TabetPos.Host/src
tags:
  - ks_host
  - vti
  - cleanup
...
Guide này dùng khi VTI đọc hoặc tiếp tục cleanup `TabetPos.Host`. Mục tiêu là làm code sạch hơn nhưng không phá luồng thiết bị legacy.
...
- Với CashChanger RT300, inventory đã lưu trong `knowledge/tabet_pos_boilerplate/devices/cash_changer_rt300.md`, gồm `TSURI_ICON.ini`, `ICON/WATCH`, commented-out result mapping, device lifecycle call và file bridge.
...
- `DeviceLog` hiện nằm trong `TabletDeviceManager` để các device project dùng chung được mà không phải duplicate helper.
- Đã áp dụng facade cho `DeviceBase`, `ScannerByDENSO`, `ScannerByDENSOForm`, `CashDrawerBySharpForm`, `LineDisplayBySharp`, `LineDisplayBySharpForm`, `CashChangerBase`, `TabletDeviceManager`, `TabletHost`, `KsClient`, `AppServer` và `AppStopServer`.
...
- Chạy `rtk codegraph sync sources/TabetPosBoilerplate` trước và sau stage cleanup.
- Build `rtk dotnet build sources/TabetPosBoilerplate/TabetPos.Host/TabetPos.Host.slnx -v:m`.
