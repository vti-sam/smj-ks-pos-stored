...
| 改訂日 | 版数 | 内容 | 改訂者 | 承認者 |
|---|---|---|---|---|
| 2026/06/11 | 0.1.0 | `device_controller_config.json` および `host_device_config.json` の記載要領を新規作成 | VTI | - |
| 2026/06/12 | 0.1.1 | デバイス種類ごとの `device_controller_config.json` 記載例を追加 | VTI | - |
| 2026/06/15 | 0.1.2 | `device_controller_config.json` の全体記載例を追加 | VTI | - |
...
| ファイル | 所在 | 用途 |
|---|---|---|
| `device_controller_config.json` | `KsPos.Applications/Resources/Raw/device_controller_config.json` | アプリに同梱する初期設定 |
| `device_controller_config.json` | `FileSystem.AppDataDirectory/device_controller_config.json` | 起動後に編集・保存する端末別設定 |
| `host_device_config.json` | `KsPos.Host/src/AppServer/Resources/host_device_config.json` | Host 側に同梱するデバイス実装設定 |
...
```mermaid
flowchart TD
    Start["タブレットPOSアプリを起動する"]
    CheckRuntime{"端末別の設定ファイルがあるか"}
    UseRuntime["端末別の設定を使用する<br/>AppData/device_controller_config.json"]
    UseDefault["アプリ同梱の初期設定を使用する<br/>Resources/Raw/device_controller_config.json"]
    ReadSetting["端末側の設定内容を読み取る"]
    Validate["使用する機器候補と有効デバイスを確認する"]
    Apply["デバイス制御で使用できる状態にする"]
    Ready["端末側のデバイス制御準備完了"]
    Error["設定に問題がある場合<br/>調査用ログを出力する"]

    Start --> CheckRuntime
    CheckRuntime -- "ある" --> UseRuntime --> ReadSetting
    CheckRuntime -- "ない" --> UseDefault --> ReadSetting
    ReadSetting --> Validate --> Apply --> Ready
    ReadSetting -. "読込不可 / 記載不正" .-> Error
    Validate -. "ID不一致 / OS不一致" .-> Error
```
...
| 配置元 | `KsPos.Host/src/AppServer/Resources/host_device_config.json` |
...
```mermaid
flowchart TD
    Start["Windows Hostを起動する"]
    UseConfig["Host実行フォルダの設定を使用する<br/>Resources/host_device_config.json"]
    ReadSetting["Host側の設定内容を読み取る"]
    ReadDevices["devices配列を上から順に確認する"]
    Validate["起動対象の機器と実装識別IDを確認する"]
    StartDevice["Host側で外部機器を起動する"]
    Ready["Host側のデバイス制御準備完了"]
    Error["設定に問題がある場合<br/>調査用ログを出力する"]

    Start --> UseConfig --> ReadSetting --> ReadDevices --> Validate --> StartDevice --> Ready
    ReadSetting -. "読込不可 / 記載不正" .-> Error
    Validate -. "ID不一致 / 実装識別ID不一致" .-> Error
```
