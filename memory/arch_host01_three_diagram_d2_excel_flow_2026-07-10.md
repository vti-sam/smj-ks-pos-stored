...
The active Basic Design workbook is the original bundle file `project-store/artifacts/reports/architecture/ARCH-HOST-01_タブレットPOS_デバイスコネクタ基本設計書/ARCH-HOST-01_タブレットPOS_デバイスコネクタ基本設計書.xlsx`; it was re-rendered in place with the version 0.2.19 Mermaid/legend changes. The earlier `draft/` workbook is only a prior preview copy.

Business Flow Excel Office Scripts now use `workbook.getActiveCell()` as the diagram anchor and the active cell's worksheet as the destination. Merged lane tables use `getOffsetRange()`/`getResizedRange()` relative to the anchor; component coordinates use the anchor top and lane-range left/width. The generator no longer looks up/creates a hard-coded worksheet, clears `B5`, unmerges the whole used range, or resizes entire worksheet columns. It clears only the calculated anchor-relative canvas and deletes only generated `shape_`, `edge_`, and `section_bg_` shapes. The current 5.1 script uses a 23-column × 37-row canvas, so selecting `B20` targets `B20:X56` and preserves the Basic Design memo at row 60. All 5.1/6.1/9.1 layout and label scripts plus the generic sample pass validators; fidelity remains 25/25 + 23/23 for 5.1, 7/7 + 9/9 for 6.1, and 12/12 + 13/13 for 9.1.

Visible connector labels now use `autoSizeShapeToFitText`, solid `#FFFFFF` fill, centered horizontal/vertical text, and a position centered on the connector shape's bounding-box midpoint. Each label explicitly calls `ShapeZOrder.bringToFront` after sizing and positioning. The blue 2pt label border remains. The generator, validator, skill contract, and every 5.1/6.1/9.1 show-label script were regenerated and validated with this behavior.
