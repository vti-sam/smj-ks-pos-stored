...
Sau khi tài liệu POS-HOST được xác định là dữ liệu chính thức gửi khách hàng, `japanese-class` cần coi cả Markdown là customer-facing deliverable: không để `CodeGraph`, `Pure Markdown`, `Evidence`, `spec-evidence`, `status: draft`, `VTI-SAM`, source-read/generator notes hoặc câu placeholder kiểu `対象 method の実行条件を確認する` trong MD/XLSX. Evidence/source line nên giữ ở audit nội bộ ngoài tài liệu giao KH.
