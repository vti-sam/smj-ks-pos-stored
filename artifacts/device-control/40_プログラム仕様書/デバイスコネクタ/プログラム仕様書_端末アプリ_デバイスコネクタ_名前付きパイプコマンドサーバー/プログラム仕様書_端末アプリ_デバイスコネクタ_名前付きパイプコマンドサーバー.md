# PS-CONNECTOR-01 タブレットPOS デバイスコネクタ 名前付きパイプコマンドサーバー プログラム仕様書
...
| 項目 | 内容 |
| --- | --- |
| 文書ID | PS-CONNECTOR-01 |
| 文書名 | タブレットPOS デバイスコネクタ 名前付きパイプコマンドサーバー プログラム仕様書 |
...
| コンストラクタ引数 | internal | string | pipeName | パイプ名 |
| コンストラクタ引数 | internal | Func<NamedPipeDeviceCommandRequest, Task<NamedPipeDeviceCommandResponse>> | handler | 要求処理ハンドラー |
...
| ⑥ | private | NamedPipeDeviceCommandRequest | ParseRequest | JSON または既存形式メッセージの要求を解析する。 |
...
- ① 旧ログ出力部へ異常メッセージ、デバイスコネクタ識別子、および異常区分を渡す。
