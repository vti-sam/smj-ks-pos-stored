# PS-CONNECTOR-04 タブレットPOS デバイスコネクタ デバイスコマンドハンドラー プログラム仕様書
...
| 項目 | 内容 |
| --- | --- |
| 文書ID | PS-CONNECTOR-04 |
| 文書名 | タブレットPOS デバイスコネクタ デバイスコマンドハンドラー プログラム仕様書 |
...
| 区分 | 可視性 | 型 | 名前 | 用途 |
| --- | --- | --- | --- | --- |
| コンストラクタ引数 | internal sealed | IDeviceRegistry | deviceRegistry | DeviceId から IFDevice を検索する依存先。 |
| コンストラクタ引数 | internal sealed | IProcessInfoStore | processInfoStore | DeviceUse/DeviceUnUse のプロセス情報更新先。 |
| コンストラクタ引数 | internal sealed | Func<bool> | readinessProbe | デバイスコネクタの準備状態を返す任意指定の確認関数。 |
...
| ③ | private | DeviceCommandResult | Failure | 検証エラーまたは未対応コマンドを、既存互換キーを含む失敗結果に変換する。 |
| ④ | private | void | EnsureLegacyReturnKeys | 戻りペイロードに ResultCode/ReturnValue が無い場合だけ、戻り値で補完する。 |
| ⑤ | private | void | LogDeviceResult | デバイス実行結果を成否に応じたログ種別で記録する。 |
...
- ③ HealthCheck は準備状態確認関数が false 以外の場合に Ready を返し、false の場合は デバイスコネクタ is not ready. の失敗結果を返す。
