...
| 項目 | 内容 |
| --- | --- |
| ソースファイル | sources/pos-integration/Pos.DeviceCtrl/DeviceManager.cs |
| 対象クラス | DeviceManager |
...
| 区分 | 可視性 | 型 | 名前 | 用途 |
| --- | --- | --- | --- | --- |
| フィールド | public const | string | DefaultConfigFileName | 実行時デバイス設定ファイル名。 |
...
| ⑦ | private | string | GetRuntimeOs | 実行中のOS識別文字列を返す。 |
| ⑧ | public | Task<IBarcodeScannerStrategy?> | GetScannerStrategyAsync | 実行環境で有効なスキャナ制御方式を取得する。 |
| ⑨ | public | Task<IPrinterStrategy?> | GetPrinterStrategyAsync | 実行環境で有効なプリンタ制御方式を取得する。 |
...
| ⑬ | public | Task<IDrawerStrategy?> | GetDrawerStrategyAsync | 実行環境で有効なドロア制御方式を取得する。 |
| ⑭ | public | Task<IKeyboardStrategy?> | GetKeyboardStrategyAsync | 実行環境で有効なキーボード制御方式を取得する。 |
| ⑮ | public | Task<IHandyTerminal?> | GetHandyTerminalAsync | 登録済みのハンディターミナルを取得する。 |
...
### ⑮. GetHandyTerminalAsync

| 項目 | 内容 |
| --- | --- |
| シグネチャ | public Task<IHandyTerminal?> GetHandyTerminalAsync() |
| 可視性 | public |
| 戻り値 | Task<IHandyTerminal?> |
| 戻り値内容 | ハンディターミナルのインスタンス。未登録の場合はnull。 |

処理内容:

- ① DIからハンディターミナルの実装を任意取得する。
- ② 取得したインスタンスを返す。未登録の場合はnullを返す。
...
備考: 対応する種別はプリンタ、スキャナ、決済端末、自動釣銭機、カスタマディスプレイ、ドロア、キーボード。
...
- ② iOSビルドではEpsonプリンタ、カメラ/BLEスキャナ、カスタマディスプレイの各制御方式を登録する。
- ③ AndroidビルドではBluetoothプリンタ、カメラスキャナ、カスタマディスプレイの各制御方式を登録する。
