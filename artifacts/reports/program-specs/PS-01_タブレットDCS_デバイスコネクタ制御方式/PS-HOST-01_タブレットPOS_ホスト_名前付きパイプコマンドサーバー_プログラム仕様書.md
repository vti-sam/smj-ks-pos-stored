...
- ① pipeName と handler をインスタンス生成時に受け取り、コマンド通信用パイプの受付先と処理委譲先を固定する。
...
- ③ 要求解析またはハンドラー実行の例外時は異常ログを出力し、ResultCode=-1 の失敗応答を生成する。
- ④ 応答を JSON で書き戻し、writer をフラッシュする。
- ⑤ 応答の送信完了後、PostWriteAction が設定されている場合は実行する。
...
- ④ 既存形式メッセージには `Legacy-` と GUID を連結した RequestId を設定する。
