...
| 期待成果 | 受信開始、停止、再接続、イベント変換および購読者通知の動作を明確にする。 |
...
Windows環境でHostプロセスのイベント用名前付きパイプへ接続し、デバイスイベントを継続的に受信するクラスである。受信したJSONをDeviceContractsのNamedPipeDeviceEventへ変換して購読者へ通知する。通信切断または解析失敗時は、通信設定または共通既定値から決定した間隔後に再接続する。
...
| イベント | public | EventHandler<NamedPipeDeviceEvent>? | EventReceived | 受信したデバイスイベントをアプリケーションへ通知する。 |
...
備考: ServiceCollectionExtensionsはIDeviceEventReceiverの実装として本クラスを登録する。
