...
デバイス設定サービスから保存場所の実装を分離するためのアクセス契約。実行時設定のパスと存在確認、実行時設定および組込み初期設定の読込、実行時設定の保存方法を定義する。
...
- 本インターフェースと実装の対応はServiceCollectionExtensionsでSingletonとして登録される。
