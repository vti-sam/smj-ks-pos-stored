...
- ① RuntimeConfigPathから実行時設定ファイルのパスを取得する。
- ② 対象ファイルの存在を確認する。
- ③ 存在確認結果を返す。
...
- ① 本クラスを含むアセンブリを取得する。
- ② DefaultResourceNameで組込みリソースのストリームを取得する。
- ③ リソースが存在しない場合はリソース名と設定ファイル名を含むFileNotFoundExceptionを送出する。
