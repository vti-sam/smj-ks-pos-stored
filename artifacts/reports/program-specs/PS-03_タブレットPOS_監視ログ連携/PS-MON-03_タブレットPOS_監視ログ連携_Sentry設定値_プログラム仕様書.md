...
| --- | --- |
| 文書ID | PS-MON-03 |
| 文書名 | タブレットPOS 監視ログ連携 Sentry設定値 プログラム仕様書 |
| 対象 | タブレットPOS / Sentry設定値 |
| 版数 | 0.0.1 |
| 作成日 | 2026/06/21 |
| 作成者 | VTI サム, VTI 吉田 |
| レビュー担当 | SMJ 蒲田 |
...
| No | 可視性 | 戻り値 | メソッド名 | 概要 |
| --- | --- | --- | --- | --- |
| ① | public static | SentrySettings | FromConfiguration | アプリ設定または環境変数からSentry設定値を生成する。 |
| ② | private static | string? | Read | 設定値を優先し、未設定の場合は環境変数を読み取る。 |
...
| 可視性 | public static |
| 戻り値 | SentrySettings |
| 戻り値内容 | アプリ設定または環境変数からSentry設定値を生成した結果。 |
...
| 可視性 | private static |
| 戻り値 | string? |
| 戻り値内容 | 設定値を優先し、未設定の場合は環境変数を読み取った結果。該当しない場合は null。 |
