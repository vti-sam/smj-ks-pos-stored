...
| 版数 | 日付 | 変更内容 | 作成者 | 承認者 |
| --- | --- | --- | --- | --- |
| 0.0.6 | 2026/07/23 | ARCH-HOST-01に合わせて、デバイスコネクタ、タブレットPOS端末アプリ、および通信経路の表記を統一 | VTI サム |  |
| 0.0.5 | 2026/07/23 | デバイスコネクタ準備状態連携と起動失敗時の停止処理を現行実装に合わせて更新 | VTI サム |  |
| 0.0.4 | 2026/06/25 | アプリライフサイクルに合わせた自動起動・停止方針を追記 | VTI サム |  |
| 0.0.3 | 2026/06/25 | 通常運用時の自動起動方針とデバッグ用Start/Stop画面の扱いを追記 | VTI サム |  |
...
| 項目 | 内容 |
| --- | --- |
| ソースファイル | sources/TabletPosBoilerplate/TabletPos.Host/src/TabletHost/DeviceHost/TabletHost.cs |
| 対象クラス | TabletHost |
