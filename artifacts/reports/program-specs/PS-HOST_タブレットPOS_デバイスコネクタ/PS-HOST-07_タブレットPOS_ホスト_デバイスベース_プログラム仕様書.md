...
| 版数 | 日付 | 変更内容 | 作成者 | 承認者 |
| --- | --- | --- | --- | --- |
| 0.0.4 | 2026/08/24 | Open・Claim状態の参照・更新メソッドと共通コマンドタイムアウト参照を現行ソースへ反映 | VTI サム | SMJ 蒲田 |
...
| 項目 | 内容 |
| --- | --- |
| ソースファイル | sources/TabletPosBoilerplate/TabletPos.Host/src/TabletDevice/DeviceBase/DeviceBase.cs |
| 対象クラス | DeviceBase |
...
| フィールド | protected | DateTime | _keepAliveDateTime | デバイス管理部の応答監視で参照する最終処理継続時刻。 |
| フィールド | protected | bool | PMntRun | timer_tick 内で Device_Mng 実行中であることを示すフラグ。 |
| プロパティ | public | TabletDeviceId | TabletDeviceId | MyDeviceId を公開するデバイス識別 ID。 |
| プロパティ | public | DateTime | KeepAliveDateTime | 最終 keep-alive 時刻を公開し、デバイス管理部の応答監視に使用する。 |
...
| ⑲ | protected | bool | IsOpened | 対象デバイスがOpen済みか確認する。 |
| ⑳ | protected | bool | IsClaimed | 対象デバイスがClaim済みか確認する。 |
| ㉑ | protected | void | SetOpened | 対象デバイスのOpen状態を更新する。 |
...
| 可視性 | public |
| 戻り値 | int |
| 戻り値内容 | Windows の前面ウィンドウハンドルを取得した結果。 |

処理内容:

- ① user32.dll の GetForegroundWindow API を呼び出す。
- ② 現在ユーザーが操作している前面ウィンドウハンドルを取得する。
- ③ 取得したハンドルを timer_tick から Device_Mng へ渡すための値として返す。
...
- ① LoopTimer の Elapsed イベントに timer_tick を登録する。
- ② interval=100ms、AutoReset=true、Enabled=true に設定し、周期処理を開始できる状態にする。
- ③ keep-alive 時刻を初期化し、デバイス管理部の応答監視に備える。
...
- ① keep-alive 時刻を現在時刻へ更新し、タイマーを一時停止する。
- ② 前面ウィンドウハンドルを取得し、MyDevice がある場合は PMntRun を true にして Device_Mng を呼ぶ。
- ③ Device_Mng 終了後に PMntRun を false に戻し、DoEvents を実行する。
- ④ EndOrder が true ならタイマーを再開せず終了する。
- ⑤ 例外時はデバイスクラス内タイマー異常としてログ出力する。
...
| 可視性 | public |
| 戻り値 | int |
| 戻り値内容 | 基底クラスでは読込データ検証を実装せず、既定値 -1。 |
...
| 可視性 | public |
| 戻り値 | int |
| 戻り値内容 | OPOS デバイスを Open し、管理状態を opened に更新した結果。 |
...
- ① TabletDeviceInfo から MyDeviceId と一致するデバイス情報を探す。
- ② シミュレーターでなければデバイス名を指定して OPOS Open を呼ぶ。
- ③ 対象デバイス情報の IsOpened を true に更新し、結果コードを返す。
...
| 可視性 | public |
| 戻り値 | int |
| 戻り値内容 | 必要に応じて Release したうえで OPOS デバイスを Close した結果。 |
...
- ① 対象デバイスが Open 済みか確認する。
- ② Claim 済み状態なら Device_Release を先に実行する。
- ③ IsOpened を false にし、シミュレーターでなければ OPOS Close を呼ぶ。
...
- ① 対象デバイス情報を探し、シミュレーターでなければDeviceContracts由来の共通コマンドタイムアウトを使用してClaimDeviceまたはClaimを呼ぶ。
...
- ① 対象デバイスが Claim 済み状態の場合だけ Release を実行する。
- ② IsClaimed を false に更新する。
- ③ シミュレーターでなければ ReleaseDevice または Release を呼び、結果コードを返す。
...
| 可視性 | public |
| 戻り値 | int |
| 戻り値内容 | Claim、DeviceEnabled、DataEventEnabled など読込開始状態を設定した結果。 |
...
| 可視性 | public |
| 戻り値 | int |
| 戻り値内容 | DataEventEnabled、DeviceEnabled、Release など読込終了状態を設定した結果。 |
...
- ④ 解放指定がある場合は Device_Release を実行し、結果コードを返す。
...
| 項目 | 内容 |
| --- | --- |
| シグネチャ | `public virtual void StartDevice(TabletDeviceId devId)` |
| 可視性 | public |
| 戻り値 | void |
| 戻り値内容 | - |
...
- ① 起動対象と内部状態を初期化する。
- ② 必要な通信部、フォーム、デバイスインスタンス、または設定オブジェクトを生成する。
...
| 項目 | 内容 |
| --- | --- |
| シグネチャ | `public virtual int DeviceMethod(TabletDeviceMethodID methodId, Dictionary<string, string> arguments, ref Dictionary<string, string> returns)` |
| 可視性 | public |
| 戻り値 | int |
| 戻り値内容 | 基底クラスではデバイス固有メソッドを実行せず、既定値 -1。 |
...
- ③ ResultCode と ResultCodeExtended などの戻り値をペイロードに設定して返却する。
...
| 可視性 | public |
| 戻り値 | int |
| 戻り値内容 | 基底クラスでは使用開始処理を実装せず、既定値 -1。 |
...
| 可視性 | public |
| 戻り値 | int |
| 戻り値内容 | 基底クラスでは使用終了処理を実装せず、既定値 -1。 |
...
| 項目 | 内容 |
| --- | --- |
| シグネチャ | `protected bool IsOpened()` |
| 可視性 | protected |
| 戻り値 | bool |
| 戻り値内容 | 対象デバイスがOpen済みの場合はtrue、それ以外はfalse。 |

処理内容:

- ① デバイス情報一覧がnullの場合はfalseを返す。
- ② 対象デバイスIDに一致し、Open済みのデバイスを検索する。
- ③ 一致した場合はtrue、見つからない場合はfalseを返す。

備考: 派生クラスが重複Openを防止するために使用する。

### ⑳. IsClaimed

| 項目 | 内容 |
| --- | --- |
| シグネチャ | `protected bool IsClaimed()` |
| 可視性 | protected |
| 戻り値 | bool |
| 戻り値内容 | 対象デバイスのClaim状態。対象がない場合はfalse。 |

処理内容:

- ① デバイス情報一覧がnullの場合はfalseを返す。
- ② 対象デバイスIDに一致するデバイスを検索する。
- ③ 一致したデバイスのClaim状態、または見つからない場合はfalseを返す。

備考: 派生クラスが占有状態を確認するために使用する。

### ㉑. SetOpened

| 項目 | 内容 |
| --- | --- |
| シグネチャ | `protected void SetOpened(bool opened)` |
| 可視性 | protected |
| 戻り値 | void |
| 戻り値内容 | - |
...
- ① デバイス情報一覧がnullの場合は処理を終了する。
- ② 対象デバイスIDに一致するデバイスを検索する。
- ③ 一致したデバイスのOpen状態を引数値へ更新する。

備考: 派生クラスが独自のOpen処理結果を共通管理状態へ反映するために使用する。
...
- 派生クラスはIsOpened、IsClaimed、SetOpenedを使用して共通のOpen・Claim状態と整合させる。
