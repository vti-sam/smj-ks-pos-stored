# PS-CONNECTOR-09 タブレットPOS デバイスコネクタ 自動釣銭機UIスレッドフォーム RT-300 プログラム仕様書
...
| 項目 | 内容 |
| --- | --- |
| 文書ID | PS-CONNECTOR-09 |
| 文書名 | タブレットPOS デバイスコネクタ 自動釣銭機UIスレッドフォーム RT-300 プログラム仕様書 |
...
| フィールド | private const | string | NameTurireqMem | 自動釣銭機要求用 memory mapped file 名（TURIREQ）。 |
| フィールド | private const | string | NameTuriansMem | 自動釣銭機応答用 memory mapped file 名（TURIANS）。 |
...
| フィールド | private const | int | ChkTimeInterval | Timer1 の監視間隔（500ms）。 |
| フィールド | private const | int | FileChkTime | ファイル連携の監視待機間隔（50ms）。 |
| フィールド | private readonly | Dictionary<string, string> | _mFileStock | ファイル連携で読み取った要求/応答情報を保持する辞書。 |
| フィールド | private const | string | NameReqfile | 自動釣銭機共通 DLL から自動釣銭機サーバーへの要求ファイル名。 |
| フィールド | private const | string | NameAnsfile | 自動釣銭機サーバーから自動釣銭機共通 DLL への応答ファイル名。 |
| フィールド | private const | string | NameReqfileOk | 要求ファイル受信確認用の OK ファイル名。 |
| フィールド | private const | string | NameAnsfileOk | 応答ファイル処理正常完了を示す OK ファイル名。 |
| フィールド | private const | string | NameAnsfileNg | 応答ファイル処理異常を示す NG ファイル名。 |
...
| --- | --- | --- |
| object | イベント発生元 | sender |
| ThreadExceptionEventArgs | 未処理例外情報 | t |
...
| 型 | 論理名 | 物理名 |
| --- | --- | --- |
| ref int | 入金金額 | lDepositAmount |
| ref string | 入金枚数データ | sDepostCounts |
| ref int | エラーコード | lErr |
| ref string | エラーメッセージ | sErr |
| ref bool | ログ出力フラグ | bLogPut |
...
| ref int | DirectIOデータ | pData |
| ref string | DirectIO文字列 | pString |
| ref int | エラーコード | lErr |
| ref string | エラーメッセージ | sErr |
...
| ref string | エラーメッセージ | sErr |
| ref int | 入金金額 | lDepositAmount |
| ref string | 入金枚数データ | sDepositCounts |
