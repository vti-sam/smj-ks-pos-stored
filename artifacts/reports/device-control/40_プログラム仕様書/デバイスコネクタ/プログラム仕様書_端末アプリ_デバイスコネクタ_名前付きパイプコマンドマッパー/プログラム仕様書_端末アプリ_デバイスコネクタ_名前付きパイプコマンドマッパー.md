# PS-CONNECTOR-12 タブレットPOS デバイスコネクタ 名前付きパイプコマンドマッパー プログラム仕様書
...
| 項目 | 内容 |
| --- | --- |
| 文書ID | PS-CONNECTOR-12 |
| 文書名 | タブレットPOS デバイスコネクタ 名前付きパイプコマンドマッパー プログラム仕様書 |
...
| 区分 | 可視性 | 型 | 名前 | 用途 |
| --- | --- | --- | --- | --- |
| コンストラクタ引数 | internal sealed | ILegacyMessageParser | legacyMessageParser | タブ区切りの既存形式メッセージを項目一覧へ変換する依存先。 |
...
| ② | public | string | GetRoutingKey | 要求形式に応じて、デバイスIDまたは「DeviceConnector」をルーティングキーとして返す。 |
| ③ | public | DeviceCommand | ToCommand | 外部要求を内部デバイスコマンドへ変換する。 |
| ④ | public | NamedPipeDeviceCommandResponse | ToResponse | 内部処理結果を外部応答へ変換する。 |
| ⑤ | private | Dictionary<string, string> | BuildItemList | 新形式要求から既存互換の項目一覧を構築する。 |
...
- ① 要求が存在しない場合は「DeviceConnector」を返す。
- ② 既存形式メッセージが空の場合は新形式要求として扱う。
- ③ 新形式要求のデバイスID（DeviceId）が設定されている場合はその値を返し、未設定の場合は「DeviceConnector」を返す。
- ④ 既存形式メッセージがある場合は項目一覧へ変換する。
- ⑤ 項目一覧に空でないデバイスID（DeviceId）がある場合はその値を返し、それ以外は「DeviceConnector」を返す。

備考: 「DeviceConnector」はデバイスIDを持たないデバイスコネクタ制御要求の共通キーとして使用する。
