# PS-CONNECTOR-08 タブレットPOS デバイスコネクタ 自動釣銭機制御 RT-300 プログラム仕様書
...
| 項目 | 内容 |
| --- | --- |
| 文書ID | PS-CONNECTOR-08 |
| 文書名 | タブレットPOS デバイスコネクタ 自動釣銭機制御 RT-300 プログラム仕様書 |
...
- デバイス使用開始時の Open 状態を確認し、強制復旧時は デバイスコネクタ 内の入金セッション状態と OPOS 接続を再構築する。

## フィールド/プロパティ

| 区分 | 可視性 | 型 | 名前 | 用途 |
| --- | --- | --- | --- | --- |
| フィールド | private static | CashChangerByRt300 | _instance | RT-300 自動釣銭機制御のシングルトンインスタンス。 |
...
| ⑧ | private | ForceRecoveryOutcome | ExecuteForceRecovery | OPOS 接続と デバイスコネクタ 内の入金セッション状態を段階的に復旧し、各ステップの結果を返す。 |
| ⑨ | private | void | ResetDepositSessionState | BeginDeposit／EndDeposit 間の デバイスコネクタ 内セッション状態を初期化する。 |
...
| 項目 | 内容 |
| --- | --- |
| シグネチャ | [DllImport("user32")] public static extern int UpdateWindow(int hwnd) |
| 可視性 | public |
| 戻り値 | int |
...
| ref Dictionary<string, string> | 応答ペイロード | returns |
...
- ④ ResetDepositSessionState で デバイスコネクタ 内の入金セッション状態を初期化する。
...
備考: デバイスコネクタ 再起動後に端末アプリケーションのセッション状態とデバイス状態を再同期するため、強制復旧フローから呼び出す。
...
| 型 | 論理名 | 物理名 |
| --- | --- | --- |
| ref string | 金種別枚数文字列 | sCashCounts |
| ref bool | 不一致有無 | bDiscrepancy |
| ref int | エラーコード | lErr |
| ref string | エラーメッセージ | sErr |
...
| 型 | 論理名 | 物理名 |
| --- | --- | --- |
| ref string | 金種別枚数出力 | pCashCounts |
| ref bool | 不一致有無 | bDiscrepancy |
