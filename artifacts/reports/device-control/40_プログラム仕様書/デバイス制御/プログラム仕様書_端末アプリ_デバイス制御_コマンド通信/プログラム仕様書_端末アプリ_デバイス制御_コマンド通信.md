# PS-DEVICE-10 タブレットPOS デバイス制御 コマンド通信 プログラム仕様書
...
| --- | --- |
| 文書ID | PS-DEVICE-10 |
| 文書名 | タブレットPOS デバイス制御 コマンド通信 プログラム仕様書 |
| 対象 | タブレットPOS / デバイスコネクタコマンド通信 |
| 版数 | 0.1.1 |
| 作成日 | 2026/08/24 |
| 作成者 | SMJサム |
| レビュー担当 | SMJ 蒲田 |
| 承認者 | - |
| 目的 | Windows環境でデバイスコネクタへデバイスコマンドを送信する名前付きパイプ通信仕様を定義する。 |
...
Windows環境でタブレットPOSからデバイスコネクタプロセスへデバイスコマンドを送信し、応答を受信する名前付きパイプクライアントである。DeviceContractsで定義した要求・応答モデルおよび共通既定値を使用し、通信設定に基づいて接続、応答待機および再試行を制御する。通信異常とデバイスコネクタ処理異常は区別して呼び出し元へ通知する。Strategy操作からDeviceId、MethodId、Payloadへの変換、およびデバイスコネクタ応答からStrategy結果・OPOSプロパティへの変換はOposNamedPipeCommandClientが担当し、本クラスは担当しない。

### 主な責務

- 名前付きパイプ設定を保持し、旧パイプ名を現行名へ補正する。
- DeviceContractsのデバイスコマンド要求をJSONへ変換してデバイスコネクタへ送信する。
- デバイスコネクタ応答を受信してコマンド応答モデルへ変換する。
- DeviceContractsの共通既定値に基づいて接続失敗時の再試行と応答待機を制御する。
- 通信例外とデバイスコネクタ処理異常を別の例外として通知する。
...
| 区分 | 可視性 | 型 | 名前 | 用途 |
| --- | --- | --- | --- | --- |
| フィールド | private const | string | LegacyDefaultPipeName | 旧版との互換判定に使用するパイプ名TabletPOSPipeMessage。 |
...
| ③ | public | NamedPipeDeviceCommandResponse | SendCommand | デバイスコマンドを送信し、デバイスコネクタ応答を検証して返す。 |
...
| ⑧ | private | bool | IsConnectionFailure | 例外が再試行可能な接続失敗か判定する。 |
...
| 項目 | 内容 |
| --- | --- |
| シグネチャ | public sealed class NamedPipeClient(IAppLogger logger) |
| 可視性 | public |
| 戻り値 | - |
| 戻り値内容 | - |
...
| 戻り値内容 | デバイスコネクタが返した成功済みのデバイスコマンド応答。 |
...
- ⑩ Successがfalseの場合はDeviceConnectorCommandExceptionを送出する。
- ⑪ 成功応答を返す。

備考: DeviceConnectorCommandExceptionはデバイスコネクタ応答全体をResponseプロパティに保持する。
...
- ③ 接続開始をデバッグログへ出力し、指定時間内にデバイスコネクタへ接続する。
