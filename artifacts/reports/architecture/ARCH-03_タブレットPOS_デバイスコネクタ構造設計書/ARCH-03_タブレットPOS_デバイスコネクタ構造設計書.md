...
- [2. 基本アーキテクチャ](#2-基本アーキテクチャ)
  ...
  - [2.4 デバイスコネクタの責務](#24-デバイスコネクタの責務)
...
- [3. 制御方式](#3-制御方式)
  ...
  - [3.4 応答通知](#34-応答通知)
- [4. デバイスコネクタ（Host）内部構成](#4-デバイスコネクタhost内部構成)
  - [4.1 制御基盤](#41-制御基盤)
...
本書は、タブレットPOS に追加するデバイスコネクタの構造を定義する構造設計書である。
...
デバイスコネクタを適用する目的は、過去のトラブル等の課題が対策済みである現行資源の有効利用の観点で、初期対象デバイスを安全に継続利用することである。

初期段階では、既存 OPOS / OCX / DLL との結合が強いデバイスをデバイスコネクタ（Host）側に配置し、タブレットPOSアプリからは Named Pipe 経由で操作要求だけを送信する。
...
| 読者 | 用途 |
|---|---|
| デバイスコネクタ開発者 | デバイスコネクタ（Host）の起動、通信、command 処理、デバイス管理の責務を確認する |
| デバイス制御層開発者 | アプリ側からデバイスコネクタ（Host）へ渡す Named Pipe 境界を確認する |
| デバイス担当者 | 初期対象デバイスと既存資源の利用範囲を確認する |
| テスト担当者 | デバイスコネクタ（Host）の起動、通信、対象デバイス呼び出し、停止時の確認観点を確認する |
...
図 2-1 に、タブレットPOSアプリ、デバイス制御層（DeviceCtrl）、デバイスコネクタ（Host）、周辺機器の関係を示す。

![デバイスコネクタ制御方式構造図](ARCH-03_タブレットPOS_デバイスコネクタ制御方式構造図.svg)

図 2-1 デバイスコネクタ制御方式構造図

図内のデバイスコネクタ（Host）側ブロック名は、対応するプログラム仕様書の機能名を示す。括弧内は実装 class 名を示す。
...
デバイス制御層（DeviceCtrl）はタブレットPOSアプリ内の層であり、別プロセスのデバイスコネクタ（Host）へ要求を渡す。
...
デバイスコネクタ（Host）は、アプリケーションから分離したプロセスとして、既存デバイス資源の起動、保持、呼び出し、終了を担当する。

デバイスコネクタ（Host）側では `TabletHost` が `TabletDeviceManager` を起動し、`host_device_config.json` から対象デバイスを読み込む。
...
通常運用時、デバイスコネクタ（Host）はユーザー操作ではなく、タブレットPOSアプリのライフサイクルに合わせて制御する。

タブレットPOSアプリの起動時、画面作成時、復帰時にはデバイスコネクタ（Host）の起動状態を確認し、未起動の場合はバックグラウンドプロセスとして自動起動する。タブレットPOSアプリの停止時、終了時にはデバイスコネクタ（Host）へ停止要求（Kill）を送信し、デバイスコネクタ（Host）側で対象デバイスと Named Pipe 通信を停止する。
...
デバイスコネクタ（Host）は `NamedPipeDeviceHostAdapter` を通じて、Command Pipe と Event Pipe を起動する。
...
`TabletHost.StartHost()` はデバイスコネクタ（Host）起動時に `DeviceHostTransport` と command handler を初期化し、`TabletDeviceManager.StartDeviceManager()` を呼び出す。

`TabletDeviceManager` は `IFSettingDevice.GetDeviceSettingList()` から起動対象を取得し、各 device class を生成して `StartDevice()` を実行する。

起動した device instance はデバイスコネクタ（Host）内の `_deviceList` に保持され、以後の command は DeviceId に基づいて該当 instance へ渡される。
...
`host_device_config.json` はデバイスコネクタ（Host）が起動するデバイス実装を定義する。
...
| 対象デバイス | デバイスコネクタ（Host）内 class | 適用理由 |
|---|---|---|
