...
旧既定値 `TabletPOSPipeMessage` が設定されている場合、`NamedPipeClient.Configure` は `TabetPos.Host.Command` へ移行する。
...
- `sources/tabletposboilerplate/TabetPos.DeviceCtrl/Models/Config/DeviceConfig.cs`
- `sources/tabletposboilerplate/TabetPos.DeviceCtrl/Models/Device/DeviceSpec.cs`
