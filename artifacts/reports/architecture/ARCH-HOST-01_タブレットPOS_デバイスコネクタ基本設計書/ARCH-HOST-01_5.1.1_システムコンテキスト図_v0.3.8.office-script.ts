function main(workbook: ExcelScript.Workbook) {
  ...
  const baselineCanvasRowCount = 13;
  formatDiagramGrid(anchor, baselineCanvasRowCount, canvasColumnCount);

  const laneRange1 = getAnchoredRange(anchor, 0, 0, 1, 25);

  const shape_d511_DEVICE = addTextShape(sheet, "shape_d511_DEVICE", "（3） 周辺機器\n自動釣銭機：GLORY RT-300／RAD-300\nドロア：SHARP UP-J46DW3\nカスタマディスプレイ：SHARP RZ-4DP3\n決済端末：CAFIS Arch Saturn", 0, 0, 202.575, 81, 10, true, "#E4DFEC", 0, "#8064A2", 2, "#111111", true, "center", "roundRect", false);
  setShapeAltText(shape_d511_DEVICE, "（3） 周辺機器 自動釣銭機：GLORY RT-300／RAD-300 ドロア：SHARP UP-J46DW3 カスタマディスプレイ：SHARP RZ-4DP3 決済端末：CAFIS Arch Saturn", "");
  ...
  let groupBottom1 = placeDiagramRow([shape_d511_APP, shape_d511_HOST, shape_d511_DEVICE], [0.15, 0.5, 0.85], laneRange1.getLeft(), originTop + 60, laneRange1.getWidth(), 30, 28);
  ...
  const edge_d511_01_APP_to_HOST = addConnector(sheet, "edge_d511_01_APP_to_HOST", shape_d511_APP, shape_d511_HOST, false, true, "コマンド通信用パイプ 要求（アプリ→コネクタ） 同期応答（コネクタ→アプリ）", "コマンド通信用パイプ 要求（アプリ→コネクタ） 同期応答（コネクタ→アプリ）", "elbow", "", "", "#1F4E79", 3);
  const edge_d511_02_HOST_to_APP = addConnector(sheet, "edge_d511_02_HOST_to_APP", shape_d511_HOST, shape_d511_APP, true, false, "イベント通知用パイプ 非同期イベント（コネクタ→アプリ）", "イベント通知用パイプ 非同期イベント（コネクタ→アプリ）", "elbow", "", "", "#C65911", 2);
