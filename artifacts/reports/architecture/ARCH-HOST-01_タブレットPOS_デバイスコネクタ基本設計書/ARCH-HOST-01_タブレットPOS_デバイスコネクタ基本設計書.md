...
| 責務領域 | 担当する内容 | 担当しない内容 |
|---|---|---|
| タブレットPOS端末アプリ | 画面・業務判断、デバイス操作の依頼、同期結果の利用、デバイスコネクタプロセスの存在確認・起動・稼働確認・停止・終了監視、およびイベント受信処理の開始・停止を自動管理する。 | OPOS、OCX、ActiveX、既存DLL、およびデバイスコネクタ内部処理の直接呼出し。現行実装では受信イベントの業務利用。 |
...
| 共通デバイス通信契約 | アプリとデバイスコネクタが共有する要求・応答・イベントDTO、識別子、ペイロードキー、および通信既定値。 | 通信処理、デバイス選択、実機制御、業務判断。 |
| 周辺機器 | 釣銭、ドロア開放、カスタマディスプレイ表示、レシート印字、およびCAFIS Arch決済などの実処理。 | アプリケーション層との直接通信。 |
...
| 10 | 通信契約の一元化 | 要求・応答・イベントDTO、識別子、ペイロードキー、および通信既定値は共通デバイス通信契約に集約し、アプリとデバイスコネクタで重複定義しない。 |
...
静的な構成要素が、起動、デバイス操作、および終了の各フェーズでどのように連携するかを示す。非同期イベントはデバイス操作フェーズ内の独立した通知経路として扱い、現行実装では受信までを対象とする。
...
- 非同期イベントは同期応答を変更しない。`NamedPipeEventReceiver`はイベント通知用パイプが切断された場合に1秒間隔で再接続する。現行コードには`EventReceived`購読先がなく、受信イベントをアプリケーション層へ引き渡さない。
...
| フェーズ | 開始条件 | 完了条件 | 主な責務 |
|---|---|---|---|
| （1）起動フェーズ | アプリを起動または再開する。 | 使用可能な設定、デバイスコネクタプロセス、デバイス管理、およびイベント受信処理の準備を確認する。 | デバイス制御設定管理、デバイスコネクタプロセス管理、イベント受信、デバイスコネクタ起動・停止管理。 |
| （2）デバイス操作フェーズ | アプリケーション層がデバイス操作を依頼する。 | 同期応答または通信失敗を返す。非同期イベントが発生した場合はイベント受信処理まで受け渡す。 | デバイス制御層、コマンド受付、順序制御、コマンド実行、デバイス管理、イベント配信、イベント受信。 |
| （3）終了フェーズ | アプリを停止または強制終了する。 | イベント受信を停止し、所有プロセスの終了確認または強制終了を完了する。 | イベント受信、デバイスコネクタプロセス管理、デバイスコネクタ起動・停止管理。 |
...
⑧ ホスト内部実装でデバイス処理結果通知が発生した場合、デバイス管理は通知データを`NamedPipeEventPublisher`へ渡す。`NamedPipeEventPublisher`はイベント通知用パイプへ送信し、`NamedPipeEventReceiver`がイベントJSONを受信して`EventReceived`を発行する。現行コードには`EventReceived`購読先がないため、受信イベントをアプリケーション層のユースケースへ通知しない。イベント通知用パイプが切断された場合は1秒間隔で再接続し、イベント通信異常は同期応答を変更しない。

関連機能: F-HOST-003、F-HOST-004、F-HOST-005、F-HOST-006、F-HOST-008、F-HOST-009、F-HOST-010

#### 6.2.4 （3）終了フェーズ

① アプリ停止または強制終了を契機に、`NamedPipeEventReceiver.StopAsync`でイベント受信処理を停止してからデバイスコネクタの終了処理を開始する。
...
| F-HOST-005 | 要求・応答変換 | 共通デバイス通信契約のJSON通信形式と内部コマンド形式を相互変換する。アプリ側では`OposNamedPipeCommandClient`がStrategy要求と応答を変換する。 | 入力: JSON要求、処理結果／出力: 内部要求、JSON応答 | 項目を内部形式へ変換し、結果を応答形式へ変換する。 | 必須情報不足は失敗応答とし、応答書込失敗時は接続を終了する。 | （1）②-2、（1）②-3、（2）②-2 |
...
デバイス処理結果通知は、個別デバイス処理が同期応答とは別に完了結果を返す場合に生成する。イベント種別（EventType）とメッセージ（Message）には、デバイス処理結果通知（ReplyDevice）を設定する。デバイス制御は受信後に`EventReceived`を発行する。現行コードには購読先がないため、デバイスID、メソッドID、ハンドル、および関連要求IDをアプリケーション層のユースケースへ引き渡さない。
...
| 7 | 付加データ（Payload） | object | 任意 | 文字列キーと文字列値で構成する戻りデータ。 | `NamedPipeEventReceiver`が`NamedPipeDeviceEvent`として受信する。現行コードではアプリケーション層へ渡さない。 |
...
| No. | メッセージ識別値 | 分類 | デバイス識別値 | メソッド識別値 | 主な用途 | 正常時の扱い | 異常時の扱い |
|---|---|---|---|---|---|---|---|
...
デバイスコネクタ再起動後、端末アプリの実機確認画面は再起動前のストラテジー参照を再利用しない。次の操作で通信切断または結果コード（ResultCode）=101を検出した場合は、操作結果を「失敗」、画面状態を「未接続」として記録し、保持していた参照を破棄する。後続操作は、使用開始（Start）により新しいデバイスコネクタセッションを確立してから実行する。
...
アプリ側設定ファイルは`device_controller_config.json`、デバイスコネクタ側設定ファイルは`host_device_config.json`である。

| 観点 | アプリ側設定ファイル | デバイスコネクタ側設定ファイル |
|---|---|---|
