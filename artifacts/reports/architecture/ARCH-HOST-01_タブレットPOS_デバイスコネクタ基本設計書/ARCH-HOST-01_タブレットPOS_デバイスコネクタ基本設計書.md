...
本書は、タブレットPOSのデバイスコネクタ（Host）を対象にした基本設計書である。タブレットPOS端末アプリは、画面・業務を担当するアプリケーション層と、デバイス制御層（DeviceCtrl）で構成される。本書の主対象は、端末アプリ内のDeviceCtrlそのものではなく、Windows端末上で動作する別プロセスのHostになる。

アプリケーション層はHostと直接通信しない。DeviceCtrlは、使用デバイスと制御方式を判断し、Host経由が必要な場合に要求を送る呼び出し側として扱う。Hostは、既存OPOS、OCX、DLL、共有メモリ、要求/応答ファイル連携を利用するデバイス実装を起動、保持、呼び出し、停止する。

### 2.2 設計方針

① アプリケーション層は、画面操作と業務処理から必要なデバイス操作をDeviceCtrlへ渡す。

② DeviceCtrlは、どの機器を使うか、Host経由で制御するかを判断する。
...
```mermaid
flowchart LR
    subgraph TERMINAL["タブレットPOS端末アプリ"]
        direction LR
    ...
    UI <-->|操作要求 / 結果| Request
    Biz <-->|業務要求 / 結果| Request
    Life <-->|起動停止要求 / 状態| Request
    Request -->|要求内容| Config
    Config -->|制御方式| Select
    Select <-->|Host経由要求 / 同期応答| Pipe
    Pipe <-->|要求 / 応答| Order
    Order <-->|順序制御 / 結果| Command
    Command <-->|デバイス検索 / 実行結果| Manager
    Command -->|入力不備・未対応| Error
    HostConfig -->|起動対象| Manager
    Manager <-->|既存実装取得 / 実行結果| Impl
    Impl <-->|制御 / 結果| CashChanger
    Impl <-->|制御 / 結果| CashDrawer
    Impl <-->|制御 / 結果| CustomerDisplay
    Impl -.->|非同期結果転送| EventOut
    EventOut -->|ReplyDevice通知| CtrlEvent
    CtrlEvent -->|イベント通知| EventIn
    Pipe -->|同期応答| CtrlResponse
    Error -->|失敗応答| CtrlResponse
    CtrlResponse -->|結果返却| Response
    Pipe -.->|受付ログ| Log
    Order -.->|順序ログ| Log
    Command -.->|処理ログ| Log
    Manager -.->|起動・検索ログ| Log
...
タブレットPOS端末アプリは、アプリケーション層とデバイス制御層（DeviceCtrl）で構成される。アプリケーション層は、画面操作または業務処理をきっかけにデバイス操作をDeviceCtrlへ依頼する。DeviceCtrlは、設定をもとに直接制御かHost経由かを判断する。アプリケーション層はHostと直接通信せず、Host経由の要求、同期応答、イベント通知はいずれもDeviceCtrlを経由する。5.1の双方向矢印は、要求送信と同期応答の戻りを同じ関係線で示す。

（1）アプリケーション層は、画面と業務処理を担当する。実機制御の詳細は持たず、必要な操作要求をDeviceCtrlへ渡す。
  ...
  ③ 起動、復帰、停止は、Hostの起動確認と停止要求のきっかけになる。
    関連シート: 09_ライフサイクル_02
  ④ 応答受信は、DeviceCtrlから同期応答の結果を受け取る。
    関連シート: 12_応答項目定義、17_エラー処理
  ⑤ イベント受信は、DeviceCtrlから非同期通知を受け取る。
    関連シート: 13_イベント項目定義、09_ライフサイクル_02

（2）デバイス制御層（DeviceCtrl）は、使用デバイスと制御方式を判断する。Host経由の対象については、名前付きパイプで要求を送信し、Hostからの同期応答とイベント通知をアプリケーション層へ返す。
  ...
  ④ 応答受信・返却は、Hostからの同期応答を受け取り、アプリケーション層へ返す。
    関連シート: 12_応答項目定義、17_エラー処理
  ⑤ イベント受信・通知は、Hostからの非同期通知を受け取り、必要に応じてアプリケーション層へ通知する。
    関連シート: 13_イベント項目定義、09_ライフサイクル_02
...
（3）デバイスコネクタ（Host）は、DeviceCtrlからの要求を受け付け、順序制御、要求振り分け、デバイス検索、Host内の個別デバイス実装呼び出し、応答返却を行う。
  ...
  ⑦ 応答生成・イベント通知部は、同期応答を通信経路へ返し、ReplyDevice系の後続通知をイベント用パイプでDeviceCtrlへ送信する。
...
通常のデバイス操作は、アプリケーション層の要求から始まり、DeviceCtrlを経由してHost内の個別デバイス実装へ渡される。同期応答はコマンド用パイプでDeviceCtrlへ返し、Host内で生成されたReplyDevice系の後続通知はイベント用パイプでDeviceCtrlへ通知する。アプリケーション層は、DeviceCtrlから結果または通知を受け取る。

① アプリケーション層は、画面操作または業務処理をきっかけにデバイス操作要求を作成し、DeviceCtrlへ渡す。
...
⑨ 通信受付部は、実行結果をコマンド応答JSONとしてDeviceCtrlへ同期返却する。DeviceCtrlは結果をアプリケーション層へ返す。

⑩ Host内でReplyDevice系の後続通知が生成された場合、イベント通知部はTabetPos.Host.Eventへ非同期通知する。DeviceCtrlはイベントを受け取り、必要に応じてアプリケーション層へ通知する。

## 07_機能一覧

| 機能ID | 機能名 | 概要 | 入力 | 出力 | 主な担当 | 備考 |
|---|---|---|---|---|---|---|
| F-HOST-001 | Host起動確認 | アプリ起動、画面作成、復帰時にDeviceCtrlがHost起動状態を確認する。 | アプリライフサイクルイベント | Host起動要否 | DeviceCtrl | 通常運用では手動Startを前提にしない。 |
...
| F-HOST-010 | エラー応答 | 入力不備、未登録デバイス、例外を失敗応答へ変換する。 | 異常情報 | 失敗応答JSON | 通信受付部、コマンド処理部、順序制御部 | 端末アプリ側の表示文言はHostでは決めない。 |

## 08_機能詳細

| 機能ID | 処理順 | 処理内容 | 正常時 | 異常時 | 主な担当 |
|---|---:|---|---|---|---|
| F-HOST-001 | 1 | DeviceCtrlがHost起動状態を確認する。 | 起動済みならそのまま処理を続ける。 | 未起動ならF-HOST-002へ進む。 | DeviceCtrl |
| F-HOST-002 | 1 | Hostプロセスをバックグラウンドで起動する。 | Hostがコマンド受付可能になる。 | 起動失敗をDeviceCtrl側ログに残し、再試行またはエラー扱いにする。 | Host本体 |
...
① アプリ起動時は、DeviceCtrlを通じてHost起動状態を確認する。未起動の場合は自動起動する。

② 画面作成時は、DeviceCtrlがデバイス利用前にHost起動状態を確認する。起動済みの場合は再起動しない。

③ アプリ復帰時は、復帰後にDeviceCtrlがHost起動状態を再確認する。未起動の場合は再起動する。

④ アプリ停止時または終了時は、DeviceCtrlがKill要求を送信する。Host側は通信部と起動済みデバイスを停止する。
...
| IF-HOST-003 | イベント通知 | デバイスコネクタ（Host） | DeviceCtrl | TabetPos.Host.Event | JSON | - | 非同期 | Host内で生成されたReplyDevice通知をDeviceCtrlが受け取り、必要に応じてアプリケーション層へ渡す。 |
...
|---|---|---|---|---|---|
| CFG-HOST-001 | device_controller_config.json | タブレットPOS端末アプリ / DeviceCtrl側 | 端末で使用するデバイス候補、有効デバイス、接続方式、名前付きパイプ設定を定義する。 | タブレットPOSアプリ起動時 | Host内で生成するデバイス実装は定義しない。 |
| CFG-HOST-002 | host_device_config.json | デバイスコネクタ（Host）側 | Hostが起動、保持する既存デバイス実装を定義する。 | Host起動時 | 端末アプリ側の有効デバイス選択は定義しない。 |
...
⑤ HostはDeviceCtrlへ技術的な結果を返す。ユーザー向け表示文言、再試行可否、業務継続可否は端末アプリ側で判断する。

### 17.2 エラー処理一覧

| エラーID | 発生箇所 | 発生条件 | 検知方法 | Host側処理 | DeviceCtrl側返却 | ログ出力 | 復旧方法 |
|---|---|---|---|---|---|---|---|
| E-HOST-001 | DeviceCtrlからHostへの接続 | Host未起動、コマンド用パイプ未待受、接続タイムアウト。 | DeviceCtrl側の接続失敗。 | Host側処理なし。 | DeviceCtrl側で接続失敗またはタイムアウト扱い。 | DeviceCtrl側接続ログ。 | Host自動起動後に再試行する。 |
...
|---|---|---|---|---|---|---|
| Q-HOST-001 | DeviceCtrl側のイベント用パイプ受信を初期対応範囲に含めるか。 | Host側はTabetPos.Host.Eventへ送信し、DeviceCtrlが受信する。アプリケーション層への通知、表示、業務処理への反映範囲はレビューで決める。 | 非同期応答、アプリ側表示、確認観点。 | SMJ / アプリ担当 | 基本設計レビュー完了前 | レビュー確認待ち |
