...
（3）デバイスコネクタ（Host）は、DeviceCtrlからの要求を受け付け、順序制御、要求振り分け、デバイス検索、実機制御呼び出し、応答返却を行う。
  ...
  ⑥ イベント通知部は、デバイス側からの後続応答を非同期通知として送信する。
    関連シート: 10_インターフェース一覧、13_イベント項目定義、18_ログ設計
...
（4）周辺機器は、店舗で利用する実機である。初期対象は、釣銭機（RT-300）、キャッシュドロア（SHARP）、カスタマーディスプレイ（SHARP）の3種類とする。
  ...
  ② 釣銭機（RT-300）は、釣銭機向けの既存資源を通して制御する。
    関連シート: 16_デバイス別設計、17_エラー処理
  ③ キャッシュドロア（SHARP）は、キャッシュドロア向けの既存資源を通して制御する。
    関連シート: 16_デバイス別設計、17_エラー処理
  ④ カスタマーディスプレイ（SHARP）は、カスタマーディスプレイ向けの既存資源を通して制御する。
    関連シート: 16_デバイス別設計、17_エラー処理
