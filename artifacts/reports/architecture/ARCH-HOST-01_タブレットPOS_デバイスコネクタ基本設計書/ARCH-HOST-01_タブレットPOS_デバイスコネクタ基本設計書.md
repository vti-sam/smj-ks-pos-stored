...
関連シート: 02_概要、06_デバイス操作要求処理フロー_02、09_ライフサイクル_02、15_設定ファイル設計、17_エラー処理

② DeviceCtrlは、使用デバイスと制御方式を判断する。Host経由の対象については、名前付きパイプで要求を送信する。
関連シート: 03_対象範囲、10_インターフェース一覧、11_要求項目定義、12_応答項目定義、13_イベント項目定義、15_設定ファイル設計

③ 通信受付部は、DeviceCtrlからのコマンド要求を受け付け、同じ接続で同期応答を返す。イベント通知とは経路を分ける。
関連シート: 06_デバイス操作要求処理フロー_02、10_インターフェース一覧、11_要求項目定義、12_応答項目定義、17_エラー処理、18_ログ設計
...
⑧ 実機は、店舗で利用する周辺機器である。初期対象は、釣銭機（RT-300）、キャッシュドロア（SHARP）、カスタマーディスプレイ（SHARP）の3種類とする。
関連シート: 03_対象範囲、16_デバイス別設計、17_エラー処理、18_ログ設計
