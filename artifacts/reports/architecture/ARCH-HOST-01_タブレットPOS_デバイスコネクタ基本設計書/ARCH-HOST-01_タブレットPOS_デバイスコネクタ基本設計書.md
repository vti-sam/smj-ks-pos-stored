...
| レビュー担当 | SMJ 鎌田 |
| 承認者 | SMJ 鎌田 |
| 関連Backlog | WBS_UI_KIBAN-209, WBS_UI_KIBAN-210, WBS_UI_KIBAN-211 |
| 目的 | デバイスコネクタ（Host）の役割、機能、インターフェース、設定、エラー処理、ログ、テスト観点を基本設計として整理する。 |
| 期待成果 | WBS_UI_KIBAN-210の基本設計書としてレビューできる状態にし、WBS_UI_KIBAN-211で確認事項を判断できるようにする。 |
| 想定形式 | Markdownを原本とし、Excel化する場合は本書の章単位をシート単位として展開する。 |

## 01_改訂履歴

| 版数 | 日付 | 変更内容 | 作成者 | 承認者 |
|---|---|---|---|---|
| 0.1.0 | 2026/07/06 | デバイスコネクタ（Host）の基本設計書として、機能一覧、インターフェース、要求/応答形式、エラー処理、ログ、設定、テスト観点を新規整理。 | VTI サム | - |
| 0.2.0 | 2026/07/07 | Excel化しやすいように、章構成をシート単位に整理。機能詳細、インターフェース、設定、エラー処理、ログ、テスト観点をID付き表形式へ見直し。 | VTI サム | - |
| 0.2.1 | 2026/07/07 | design-doc-mdの基本設計Markdown gateに合わせ、概要と対象範囲の章番号、Backlog trace、未決事項の管理項目、表記を見直し。 | VTI サム | SMJ 鎌田 |
| 0.2.2 | 2026/07/07 | Excel出力時に読みやすくなるよう、インライン装飾を削除し、主要章では論理的な構成要素名を使用。物理クラス名は実装対応表へ分離。 | VTI サム | SMJ 鎌田 |
| 0.2.3 | 2026/07/07 | 基本設計として読みやすくするため、概要、対象範囲、全体構成、処理フロー、ライフサイクル、方針系シートを文章、番号付きステップ、Mermaid図中心の構成へ見直し。Excel出力範囲をA列からX列まで広げる前提に変更。 | VTI サム | SMJ 鎌田 |
| 0.2.4 | 2026/07/08 | 05_全体構成に設計書全体マップと構成ブロック別の対応シートを追加。後続シートが、タブレットPOSアプリ、デバイス制御層（DeviceCtrl）、デバイスコネクタ（Host）、周辺機器のどこを説明するか分かるように見直し。 | VTI サム | SMJ 鎌田 |
...
⑤ 物理クラス名や詳細メソッドは、本文で繰り返さず、24_実装対応表またはプログラム仕様書で確認する。

### 2.3 レビュー判断

本書の対象範囲、役割分担、通信方式、設定、エラー処理、ログ、テスト観点に大きな認識差異がなければ、WBS_UI_KIBAN-210をレビュー対象として扱う。未決事項は、WBS_UI_KIBAN-211で確認する。
...
| WBS_UI_KIBAN-209 | 【WBS 3.5.4.1】デバイスコネクタ対応方針整理 | 本書の前提となる対応方針整理。 |
| WBS_UI_KIBAN-210 | 【WBS 3.5.4.2】基本設計書（デバイスコネクタ） | 本書の作成対象課題。 |
| WBS_UI_KIBAN-211 | 【WBS 3.5.4.3】基本設計レビュー（デバイスコネクタ） | 本書のレビューと未決事項確認の追跡課題。 |

## 05_全体構成

### 5.1 設計書全体マップ

本シートでは、後続シートを読む前に、本書の構成を4つの主なブロックに分けて確認できるようにする。主なブロックは、タブレットPOSアプリ、デバイス制御層（DeviceCtrl）、デバイスコネクタ（Host）、周辺機器である。

設計書の管理情報、対象範囲、関連資料、Excel化方針、未決事項は、特定の技術ブロックだけに閉じないため、設計書全体の確認情報として扱う。

```mermaid
flowchart LR
    DOC["設計書全体<br/>00_表紙、01_改訂履歴、02_概要、03_対象範囲、04_関連資料、21_Excel化方針、22_Excelシート構成、23_未決事項"]
    APP["タブレットPOSアプリ<br/>画面と業務処理から機器操作を要求する"]
    CTRL["デバイス制御層（DeviceCtrl）<br/>使用デバイスと制御方式を判断する"]
    HOST["デバイスコネクタ（Host）<br/>既存資源を使う機器制御を別プロセスで実行する"]
    DEV["周辺機器<br/>直接制御機器とHost経由の実機を扱う"]
    DOC --> APP --> CTRL --> HOST --> DEV
```

### 5.2 構成ブロックと対応シート

各シートは、以下の構成ブロックに対応する。1つのシートが複数ブロックに関係する場合は、関係するすべてのブロックへ対応付ける。

| 構成ブロック | 役割 | 対応シート |
|---|---|---|
| 設計書全体 | 文書の目的、対象範囲、関連資料、Excel化方針、未決事項を確認する。 | 00_表紙、01_改訂履歴、02_概要、03_対象範囲、04_関連資料、21_Excel化方針、22_Excelシート構成、23_未決事項 |
| タブレットPOSアプリ | 画面または業務処理から機器操作を要求し、Hostの起動確認や停止要求を行う。 | 02_概要、05_全体構成、06_処理フロー、09_ライフサイクル、15_設定ファイル設計、17_エラー処理、19_テスト観点、20_テスト分類 |
| デバイス制御層（DeviceCtrl） | 使用デバイスと制御方式を判断し、Host経由の場合は名前付きパイプで要求を送る。 | 03_対象範囲、05_全体構成、06_処理フロー、10_インターフェース一覧、11_要求項目定義、12_応答項目定義、13_イベント項目定義、15_設定ファイル設計、17_エラー処理、19_テスト観点、20_テスト分類 |
| デバイスコネクタ（Host） | 要求受付、順序制御、要求振り分け、デバイス管理、既存資源呼び出し、停止、ログ出力を担当する。 | 02_概要、03_対象範囲、05_全体構成、06_処理フロー、07_機能一覧、08_機能詳細、09_ライフサイクル、10_インターフェース一覧、11_要求項目定義、12_応答項目定義、13_イベント項目定義、14_メッセージ一覧、15_設定ファイル設計、16_デバイス別設計、17_エラー処理、18_ログ設計、19_テスト観点、20_テスト分類、24_実装対応表 |
| 周辺機器 | 直接制御する機器とHost経由の実機を分けて扱い、初期対象機器の制御範囲を確認する。 | 03_対象範囲、05_全体構成、16_デバイス別設計、17_エラー処理、18_ログ設計、19_テスト観点、20_テスト分類 |
...
① タブレットPOSアプリは、画面と業務処理を担当する。実機制御の詳細は持たず、必要な操作要求をDeviceCtrlへ渡す。

② DeviceCtrlは、使用デバイスと制御方式を判断する。Host経由の対象については、名前付きパイプで要求を送信する。

③ 通信受付部は、DeviceCtrlからのコマンド要求を受け付け、同じ接続で同期応答を返す。イベント通知とは経路を分ける。

④ 順序制御部は、同一DeviceIdの要求順を守る。これにより、同一実機への多重操作や順序入れ替わりを避ける。

⑤ コマンド処理部は、Host制御要求とデバイス操作要求を振り分ける。デバイス操作の場合は、デバイス検索と実行へ進める。

⑥ デバイス管理部は、host_device_config.jsonを読み込み、Host内で起動するデバイスを生成、保持、検索、停止する。

⑦ 個別デバイス実装は、OPOS、OCX、DLL、共有メモリ、要求/応答ファイル連携などの既存資源を使い、実機制御を行う。

⑧ 実機は、店舗で利用する周辺機器である。初期対象は、釣銭機、キャッシュドロア、カスタマーディスプレイの3種類とする。

## 06_処理フロー

### 6.1 通常処理

通常のデバイス操作は、アプリ側の要求から始まり、Host内で対象デバイスへ渡される。同期応答はコマンド用パイプで返し、デバイス側から後続で返る応答はイベント用パイプで通知する。
...
⑩ デバイス側から後続応答が返る場合、イベント通知部はTabetPos.Host.Eventへ非同期通知する。
...
| F-HOST-004 | イベント通知 | デバイス側からの後続応答をイベントとして送信する。 | デバイス側応答 | イベント通知JSON | イベント通知部 | 同期応答とは別経路。 |
...
| F-HOST-004 | 1 | デバイス側から後続応答を受け取る。 | イベント通知データを作成する。 | 送信先がない場合はログを残し、同期応答とは分けて扱う。 | イベント通知部 |
...
| F-HOST-008 | 3 | 対象デバイスへ処理を渡す。 | DeviceUse、DeviceUnUse、DeviceMethodの結果を受け取る。 | デバイス側戻り値または例外を失敗応答へ変換する。 | 個別デバイス実装 |
...
| IF-HOST-003 | イベント通知 | デバイスコネクタ（Host） | DeviceCtrlまたはアプリ側 | TabetPos.Host.Event | JSON | - | 非同期 | デバイス側から後続で返る応答を扱う。 |
...
| EVT-HOST-007 | payload | object | 任意 | デバイス側から返るデータ。 | 個人情報や決済機微情報はログへそのまま出さない。 |
...
|---|---|---|---|---|---|---|---|---|
| DEV-HOST-001 | RT-300釣銭機 | Host経由 | 釣銭機制御部 | 入金、払出、状態確認、エラー確認。 | host_device_config.jsonに定義され、Host起動時に対象となる。 | Host停止時またはデバイス停止処理時。 | OPOS、OCX、UIスレッド、共有メモリ、要求/応答ファイル連携。 | 同期応答と非同期応答を分けて扱う。 |
| DEV-HOST-002 | SHARPキャッシュドロア | Host経由 | キャッシュドロア制御部 | ドロアオープン、状態確認。 | host_device_config.jsonに定義され、Host起動時に対象となる。 | Host停止時またはデバイス停止処理時。 | SHARP既存実装、OPOS、OCX。 | 結果コードを呼出元へ返す。 |
| DEV-HOST-003 | SHARPカスタマーディスプレイ | Host経由 | カスタマーディスプレイ制御部 | 表示、消去、スクロール、位置指定表示。 | host_device_config.jsonに定義され、Host起動時に対象となる。 | Host停止時またはデバイス停止処理時。 | SHARP既存実装、OPOS、OCX、日本語文字列。 | 名前付きパイプではUTF-8を前提に扱う。 |
...
| 観点ID | 確認対象 | 確認内容 | 期待結果 | 関連機能ID |
|---|---|---|---|---|
| T-HOST-001 | Host自動起動 | アプリ起動、画面作成、復帰時にHostが未起動なら自動起動すること。 | Hostが起動し、コマンド受付可能になる。 | F-HOST-001, F-HOST-002 |
| T-HOST-002 | Host停止 | アプリ停止、終了時にKillが送信されること。 | Hostがパイプと起動済みデバイスを停止する。 | F-HOST-007 |
| T-HOST-003 | コマンド疎通 | DeviceCtrlからTabetPos.Host.Commandへ要求を送ること。 | 同期応答を受け取れる。 | F-HOST-003 |
| T-HOST-004 | JSON解析 | 大小文字差異を含むJSON要求を解析すること。 | Host内部で扱う要求情報へ変換できる。 | F-HOST-005 |
| T-HOST-005 | 既存形式受付 | JSON以外の既存形式メッセージを受け付けること。 | Host内部で扱う要求情報へ変換できる。 | F-HOST-005 |
| T-HOST-006 | 入力検証 | 空行、deviceId未指定、methodId未指定を送ること。 | 失敗応答を返す。 | F-HOST-010 |
| T-HOST-007 | 未登録デバイス | 未登録DeviceIdを指定すること。 | 未登録デバイスとして失敗応答を返す。 | F-HOST-008, F-HOST-010 |
| T-HOST-008 | 順序制御 | 同一DeviceIdへ複数要求を投入すること。 | 重複実行されず、投入順に処理される。 | F-HOST-006 |
| T-HOST-009 | デバイス呼び出し | DeviceUse、DeviceUnUse、DeviceMethodを送ること。 | 対象デバイスへ正しく渡る。 | F-HOST-008 |
| T-HOST-010 | イベント通知 | デバイス応答を発生させること。 | TabetPos.Host.Eventへイベント通知される。 | F-HOST-004 |
| T-HOST-011 | 設定読込 | host_device_config.jsonに対象デバイスを定義すること。 | デバイス管理部に登録される。 | F-HOST-009 |
| T-HOST-012 | 実機連携 | RT-300、SHARPキャッシュドロア、SHARPカスタマーディスプレイの主要操作を確認すること。 | 実機操作結果が応答として返る。 | F-HOST-008 |
| T-HOST-013 | 接続失敗 | Host未起動または停止後に要求を送ること。 | アプリ側がタイムアウトまたは接続失敗として扱える。 | F-HOST-003, F-HOST-010 |
| T-HOST-014 | 再起動 | Host停止後に再起動すること。 | 再度コマンド疎通できる。 | F-HOST-002, F-HOST-007 |

## 20_テスト分類

| 分類ID | 種別 | 確認対象 | 確認内容 | 対応観点ID |
|---|---|---|---|---|
| TT-HOST-001 | 単体 | コマンド変換部 | 要求オブジェクト、既存形式メッセージ、定義されていないID、既定値補完を確認する。 | T-HOST-004, T-HOST-005 |
| TT-HOST-002 | 単体 | コマンド処理部 | Kill、ReStart、入力検証、デバイス未登録、DeviceMethod結果を確認する。 | T-HOST-006, T-HOST-007, T-HOST-009 |
| TT-HOST-003 | 単体 | 順序制御部 | 同一DeviceId順序制御、例外応答化、Host共通経路を確認する。 | T-HOST-008 |
| TT-HOST-004 | 単体 | 通信受付部 | JSON解析、既存形式受付、空行失敗応答を確認する。 | T-HOST-003, T-HOST-004, T-HOST-005 |
| TT-HOST-005 | 単体 | イベント通知部 | 接続済みクライアントへのイベント送信を確認する。 | T-HOST-010 |
| TT-HOST-006 | 結合 | DeviceCtrl連携 | アプリ側からHostへ要求を送信し、同期応答を受け取ることを確認する。 | T-HOST-003 |
| TT-HOST-007 | 結合 | Hostプロセス | 実行環境で起動し、コマンド用パイプを待ち受けることを確認する。 | T-HOST-001 |
| TT-HOST-008 | 結合 | 実機連携 | 初期対象デバイスの主要操作とエラー時応答を確認する。 | T-HOST-012 |
| TT-HOST-009 | 結合 | 停止/再起動 | Killによる停止後、再起動して再度疎通できることを確認する。 | T-HOST-002, T-HOST-014 |
| TT-HOST-010 | 結合 | エラー復旧 | Host未起動、パイプ切断、実機異常、設定不備を識別できることを確認する。 | T-HOST-006, T-HOST-007, T-HOST-013 |

## 21_Excel化方針

### 21.1 レイアウト方針

Excelでは、全シートの本文領域をA列からX列まで使用する。説明文、Mermaidブロック、コードブロックはA列からX列まで広げて表示する。

### 21.2 記載方針

① 章をシート単位にする。00_表紙から24_実装対応表までを、そのままExcelのシート名として利用できる構成にする。

② 1シート1主題にする。一つのシートでは一つの観点だけを扱い、機能、詳細、設定、エラー、ログ、テストを混在させない。

③ すべてを表にしない。概要、対象範囲、全体構成、処理フロー、ライフサイクル、方針説明は、文章、番号付きステップ、Mermaid図を使って読みやすくする。

④ 表は、ID付きの一覧、項目定義、設定、エラー一覧、ログ一覧、テスト観点、未決事項、実装対応表など、行単位でレビューする情報に使う。

⑤ 主要章では、通信受付部、順序制御部、コマンド処理部、デバイス管理部などの役割名を使用する。物理クラス名は24_実装対応表にまとめる。

⑥ ExcelではMarkdownのインライン装飾が再現されないため、表や文章ではバッククォート表記を使わない。

⑦ 詳しい実装ロジックは本文に詰め込まず、プログラム仕様書に分ける。

⑧ 未決事項は本文に混ぜず、最後の23_未決事項で管理する。

## 22_Excelシート構成

| シート名 | 元章 | 内容 | 主キー |
|---|---|---|---|
| 00_表紙 | 00 | 文書ID、文書名、対象、版数、作成日。 | 項目 |
| 01_改訂履歴 | 01 | 版数、日付、変更内容、作成者、承認者。 | 版数 |
| 02_概要 | 02 | 本書の位置づけ、設計方針、レビュー判断。 | 文章 + 番号付きステップ |
| 03_対象範囲 | 03 | 対象範囲と対象外範囲。 | 文章 + 番号付きステップ |
| 04_関連資料 | 04 | 関連する設計書、設定資料、プログラム仕様書、Backlog課題。 | 文書ID |
| 05_全体構成 | 05 | 設計書全体マップ、構成ブロックと対応シート、構成要素、役割、全体関係図。 | Mermaid + 対応シート表 + 番号付きステップ |
| 06_処理フロー | 06 | 要求から応答までの処理順。 | Mermaid + 番号付きステップ |
| 07_機能一覧 | 07 | Host機能の一覧。 | 機能ID |
| 08_機能詳細 | 08 | 機能ごとの処理内容、正常時、異常時。 | 機能ID + 処理順 |
| 09_ライフサイクル | 09 | 起動、復帰、停止、終了、デバッグ時の扱い。 | Mermaid + 番号付きステップ |
| 10_インターフェース一覧 | 10 | 名前付きパイプの一覧。 | IF-ID |
| 11_要求項目定義 | 11 | コマンド要求JSONの項目定義。 | 項目ID |
| 12_応答項目定義 | 12 | コマンド応答JSONの項目定義。 | 項目ID |
| 13_イベント項目定義 | 13 | イベント通知JSONの項目定義。 | 項目ID |
| 14_メッセージ一覧 | 14 | message種別と必須項目。 | メッセージID |
| 15_設定ファイル設計 | 15 | 設定ファイルと設定項目。 | 設定ID |
| 16_デバイス別設計 | 16 | 初期対象デバイス別の設計。 | デバイスID |
| 17_エラー処理 | 17 | エラー方針とエラー処理一覧。 | 方針説明 + エラーID |
| 18_ログ設計 | 18 | ログ方針とログ出力一覧。 | 方針説明 + ログID |
| 19_テスト観点 | 19 | テスト観点一覧。 | 観点ID |
| 20_テスト分類 | 20 | 単体、結合のテスト分類。 | 分類ID |
| 21_Excel化方針 | 21 | Excel化するときのレイアウト方針。 | 文章 + 番号付きステップ |
| 22_Excelシート構成 | 22 | Excel化する場合のシート一覧と主キー。 | シート名 |
| 23_未決事項 | 23 | 確認事項、影響範囲、確認先、ステータス。 | 確認ID |
| 24_実装対応表 | 24 | 論理的な構成要素と物理クラス名の対応。 | 対応ID |

## 23_未決事項

| 確認ID | 確認事項 | 現時点の前提 | 影響範囲 | 確認先 | 確認期限 | 追跡課題 | ステータス |
|---|---|---|---|---|---|---|---|
| Q-HOST-001 | DeviceCtrl側のイベント用パイプ受信を初期対応範囲に含めるか。 | Host側はTabetPos.Host.Eventへ送信する。アプリ側の受信、表示、業務処理への反映範囲はレビューで決める。 | 非同期応答、アプリ側表示、テスト観点。 | SMJ / アプリ担当 | WBS_UI_KIBAN-211完了前 | WBS_UI_KIBAN-211 | レビュー確認待ち |
| Q-HOST-002 | Host接続タイムアウト値の既定値をどこで管理するか。 | device_controller_config.json側の名前付きパイプ設定に寄せる想定。画面表示文言と再試行条件はアプリ側で判断する。 | 接続失敗時の応答、画面表示、再試行。 | SMJ / アプリ担当 | WBS_UI_KIBAN-211完了前 | WBS_UI_KIBAN-211 | レビュー確認待ち |
| Q-HOST-003 | Start/Stop画面を通常運用で非表示にするか。 | 通常運用では自動起動、自動停止を前提にする。Start/Stop画面はデバッグまたは開発者確認用に限定する想定。 | 運用手順、デバッグ方法。 | SMJ | WBS_UI_KIBAN-211完了前 | WBS_UI_KIBAN-209, WBS_UI_KIBAN-211 | レビュー確認待ち |
| Q-HOST-004 | エラー時のユーザー表示文言をHost応答からどう変換するか。 | Hostは技術的な結果コードとメッセージを返す。ユーザー向け表示文言、再試行可否、業務継続可否はアプリ側で整理する。 | アプリ側エラー表示、業務継続判断。 | SMJ / 業務担当 | WBS_UI_KIBAN-211完了前 | WBS_UI_KIBAN-211 | レビュー確認待ち |
| Q-HOST-005 | プリンターなど将来追加機器をHost経由に含める条件。 | 初期対象はRT-300釣銭機、SHARPキャッシュドロア、SHARPカスタマーディスプレイに限定する。追加機器は対応方針整理後に本書へ反映する。 | 設定、機能一覧、エラー処理、テスト観点。 | SMJ / デバイス担当 | WBS_UI_KIBAN-210完了後の追加方針整理時 | WBS_UI_KIBAN-209, WBS_UI_KIBAN-210 | 方針整理待ち |
