...
| 版数 | 日付 | 変更内容 | 作成者 | 承認者 |
|---|---|---|---|---|
| 0.2.19 | 2026/07/10 | Mermaidの構成要素を角丸ブロックに統一し、判定・分岐のみをひし形で表すルールへ統一。 | VTI サム | - |
| 0.2.18 | 2026/07/10 | 全体構成図の判定・分岐点をひし形で表し、凡例に表示ルールを追加。 | VTI サム | - |
...
| 表示 | 色 | 意味 |
|---|---|---|
| （1）タブレットPOS端末アプリ | #F7FBFF | アプリケーション層とデバイス制御を含む端末アプリの責務範囲を示す。 |
| アプリケーション層 | #DDEBF7 | 画面・業務処理、アプリライフサイクル、およびデバイスコネクタプロセス管理を示す。 |
| デバイス制御 | #E2F0D9 | 設定読込、使用ストラテジー判定、要求生成、名前付きパイプ通信、および応答返却を示す。 |
| （2）デバイスコネクタ | #FCE4D6 | デバイスコネクタの起動・停止、通信処理、デバイス管理、実機制御、およびイベント配信を示す。 |
| （3）周辺機器 | #E4DFEC | デバイスコネクタから制御される周辺機器を示す。 |
| 角丸ブロック（構成要素①〜⑲） | #F8FBFD | 各領域の処理、管理機能、および対象デバイスを示す。 |
| ◇ | #F8FBFD | 処理方針または実行先を選択する判定・分岐点を示す。 |
| ━━▶ ライフサイクル／制御 | #548235 | 起動、停止、存在確認、および管理制御の一方向フローを示す。 |
| ◀━━▶ 要求／応答 | #1F4E79 | アプリとデバイスコネクタ間の同期要求と応答を示す。 |
| ◀━━▶ 実機制御／結果 | #7030A0 | デバイス実装と周辺機器間の制御と結果を示す。 |
| ┄┄▶ 非同期イベント | #C65911 | デバイス処理結果の非同期通知を示す。 |
| ┈┈▶ 設定参照 | #7F7F7F | 設定情報の参照関係を示す。 |
