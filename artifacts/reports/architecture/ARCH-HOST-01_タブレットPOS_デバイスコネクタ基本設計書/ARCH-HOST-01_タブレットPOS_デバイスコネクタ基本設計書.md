...
| local_display | customer_display_sharp_windows | カスタマーディスプレイ（CustomerDisplay） | CustomerDisplay / CustomerDisplay1 |
...
| 3 | SHARPカスタマーディスプレイ | カスタマーディスプレイ制御部 | 表示、消去、スクロール、位置指定表示。 | host_device_config.jsonにカスタマーディスプレイ（CustomerDisplay） / CustomerDisplay1として定義されている。 | デバイスコネクタ停止時またはデバイス停止処理時。 | デバイスコネクタ内SHARP既存実装、OPOS、OCXを保持する非表示フォーム、日本語文字列。 | 名前付きパイプはUTF-8の一行単位で送受信する。 |
