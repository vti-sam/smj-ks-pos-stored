...
| 2026/06/23 | 1.0.0 | TabletClient 旧方式の削除、AppStopServer の Named Pipe 停止要求、Host 構成図を現行方式に合わせて更新 | VTI    | -      |
|            |       |                                                                                                                                                                          |        |        |
...
- Application-Host 通信経路は `TabetPos.DeviceCtrl` から Named Pipe で `TabetPos.Host` を呼び出す方式とする。
- 旧方式の中継DLLである `TabletClient` は使用しない。Application から Host へ直接旧方式のメッセージを送信する経路は設計対象外とする。
...
- Host は Named Pipe command経路とHost側Event publisherを提供する。DeviceCtrl側Event listenerは未実装である。
...
| デバイスHostプロセス | `TabetPos.Host` | MAUI app とは別プロセスで起動する Named Pipe host。device manager、OPOS / OCX adapter を保持する |
| Legacy binaries | `TabetPos.Host/legacyBin` | vendor / device interop DLL |
...
- AppStopServer から送信される Host 停止要求の受付。
...
`AppServer` は `TabetPos.Host` process の entry point である。debug mode は `ServerAppForm` を表示し、開始 / 終了ボタンで Host を制御する。non-debug mode では `HostProcessRuntime` が `TabletHost` を起動し、`IsEndOrder` まで WinForms message pump を保持する。
...
| `devices[].classId` | legacy `KsClassFactory` で生成する runtime class ID |
| `devices[].productName` | device product name |

`LineDisplay1` は legacy API の `KsClassID` / `KsClassFactory` が使用する物理実装側の class IDである。Application / DeviceCtrl / Host間の論理IDは `CustomerDisplay` に統一するが、この class ID と legacy API 名は変更しない。
...
Host 内部では、Named Pipe transport と device command handling を分離する。Named Pipe request は `NamedPipeCommandMapper` で内部コマンドへ変換し、`DeviceCommandHandler` へ渡す。
...
| `NamedPipeCommandMapper` | Named Pipe request / response と Host 内部コマンドの相互変換 |
...
`TabletDeviceManager` は loaded device の snapshot を公開し、device lookup は `FindDevice(TabletDeviceId)` 経由で行う。Host command handler はこの registry interface を通じて `IFDevice` を取得し、mutable list へ直接依存しない。

`TabletProcessInfo` は `(deviceId, handle)` をkeyにした thread-safe store として保持する。CashChanger などのlegacy deviceがstatic facadeを参照するため、`ProcessInfoes` などの既存 API は当面互換として残す。
...
Host は `message`, `deviceId`, `methodId`, `handle`, `payload` をもとに内部コマンドを組み立てる。
...
デバイス側が `ReplyDevice(...)` を呼び出すと、Host は `TabetPos.Host.Event` へNamed Pipeイベントをpublishする。現時点ではDeviceCtrl側listenerが未実装のため、イベントのend-to-end受信は成立していない。
...
| `AppStopServer` | レガシー終了要求をNamed Pipe経由で送信 |
...
commit `b484ac1` 以降、Host 側の自社管理クラス / assembly 名は Tablet prefix に統一する。一方、`legacyBin` が提供する `KsUtility`, `KsLogControls`, `KsClassID`, `KsClassFactory`, `KsMemoryMappedFile`, `KsApplicationControls`, `KSINIED` などは外部 binary API 名であり、build / runtime reference のため変更しない。
...
| `TabetPos.Host/tests/TabletDeviceSettingBase.Tests` | Host device config、class ID、OPOS runtime connectivityを検証。実機Openは `TABETPOS_OPOS_RUN_HARDWARE_OPEN_TESTS=1` 指定時のみ実行。2026/06/23 のローカル確認では build 成功。`dotnet test` は macOS arm64 に `Microsoft.WindowsDesktop.App 10.0.0` がないため testhost 起動前に停止 |
