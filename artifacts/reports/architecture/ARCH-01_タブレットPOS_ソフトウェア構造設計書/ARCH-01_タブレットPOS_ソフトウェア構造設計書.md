...
- [4. デバイス制御層アーキテクチャ](#4-デバイス制御層アーキテクチャ)
 - [4.1 位置づけ](#41-位置づけ)
 - [4.2 レイヤ構成](#42-レイヤ構成)
 - [4.3 設計要素](#43-設計要素)
- - [4.4 デバイスコネクタ連携方式](#44-デバイスコネクタ連携方式)
 - [4.5 プラットフォーム別ストラテジーとデバイスコネクタコマンドの対応](#45-プラットフォーム別ストラテジーとデバイスコネクタコマンドの対応)
 - [4.5.1 デバイスコネクタ経由デバイス一覧](#451-デバイスコネクタ経由デバイス一覧)
 - [4.5.2 カスタマディスプレイ](#452-カスタマディスプレイ)
 - [4.5.3 ドロア](#453-ドロア)
 - [4.5.4 自動釣銭機](#454-釣銭機)
 - [4.5.5 プラットフォーム内ストラテジー構成](#455-プラットフォーム内ストラテジー構成)
 - [4.6 設定方針](#46-設定方針)
 - [4.7 実装段階](#47-実装段階)
- [5. デバイスコネクタアーキテクチャ](#5-デバイスコネクタアーキテクチャ)
 - [5.1 デバイスコネクタの役割](#51-デバイスコネクタの役割)
 - [5.2 起動フロー](#52-起動フロー)
 - [5.3 停止フロー](#53-停止フロー)
 - [5.4 デバイスコネクタ設定ファイル](#54-デバイスコネクタ設定ファイル)
 - [5.5 デバイスインターフェース境界](#55-デバイスインターフェース境界)
 - [5.6 デバイスコマンドルーター](#56-デバイスコマンドルーター)
 - [5.7 既存デバイス資源境界](#57-既存デバイス資源境界)
 - [5.8 デバイス別制約](#58-デバイス別制約)
- [6. 端末アプリケーション・デバイスコネクタ連携方式](#6-端末アプリケーションホスト連携方式)
 - [6.1 基本方針](#61-基本方針)
 - [6.2 パイプ一覧](#62-パイプ一覧)
 - [6.3 コマンド要求形式](#63-コマンド要求形式)
 - [6.4 コマンド応答形式](#64-コマンド応答形式)
 - [6.5 対応メッセージ](#65-対応メッセージ)
 - [6.6 イベント方式](#66-イベント方式)
 - [6.7 デバイス制御呼び出し構造](#67-デバイス制御呼び出し構造)
- [7. 共通基盤方式](#7-共通基盤方式)
 - [7.1 DI](#71-di)
...
- 端末アプリケーションは `Pos.Applications` を中心とする .NET MAUI アプリケーションである。
- WindowsのCustomerDisplay、CashDrawer、CashChanger、Printer、およびPaymentは `Pos.DeviceConnector` を経由して制御する。ScannerとKeyboardはデバイスコネクタ（Host）を経由せず `Pos.DeviceCtrl` 内でローカル制御する。
- `Pos.DeviceConnector` は OPOS / OCX / ActiveX を利用するデバイスコネクタ（Host）経由デバイスを扱う Windows Forms プロセスである。
- WindowsのPrinterとPaymentはデバイスコネクタ（Host）の5デバイス構成に含める。
- Application とデバイスコネクタ（Host）の通信経路は `Pos.DeviceCtrl` から Named Pipe で `Pos.DeviceConnector` を呼び出す方式とする。
- Application はデバイス制御層（DeviceCtrl）を経由してデバイスコネクタ（Host）へ要求を送信する。
- `Pos.Applications` は業務DomainまたはAPI clientを含まず、名前付きパイプ通信を `Pos.DeviceCtrl` に委譲する。業務機能を追加する場合も、抽象境界と実装境界を分離する。
...
| デバイスコネクタ基本設計書 |
| DB-DEVICE-01_デバイス制御設定_SQLiteテーブル定義書_Ver0.1.0.xlsx |
...
- デバイスstrategy / factory / config は `Pos.DeviceCtrl` レイヤに分離し、`Pos.Applications` へ直接埋め込まない。
- `Pos.DeviceCtrl` は `TabletPos.Core.Logging.IAppLogger` を共通ログポートとして利用し、デバイス制御層（DeviceCtrl）内に独自 logger 基盤を持たない。
- iOS / Android の Epson SDK binding は `Pos.BindingLibrary` に分離する。
- `Composition` で DI 登録を行う。
- `Pos.DeviceConnector` は MAUI application の内部レイヤではなく、別プロセスとして起動する Named Pipe host である。
...
`Pos.Applications` は OS 条件により target framework を切り替える。

| プロジェクト | 動作基盤 |
|---|---|
| `Pos.Applications` | Windows: `net10.0-windows10.0.19041.0` |
| `Pos.Applications` | non-Windows placeholder: `net10.0-ios` |
| `TabletPos.Core` | Windows: `net10.0-windows10.0.19041.0` / non-Windows: `net10.0-android`, `net10.0-ios` |
| `Pos.DeviceCtrl` | .NET class library / MAUI互換デバイス制御層（DeviceCtrl） |
| `Pos.BindingLibrary` | iOS / Android Epson SDK binding |
| `Pos.ApplicationControls` | MAUIコントロールプロジェクト |
| `Pos.DeviceConnector` | Windows Forms / 外部Named Pipeデバイスコネクタ（Host）プロセス |
...
| 分類 | プロジェクト／モジュール | 概要 |
|---|---|---|
| 端末アプリケーション | `Pos.Applications` | POS 画面、ViewModel、画面遷移、Applicationユースケース |
| 共通基盤 | `TabletPos.Core` | IAppLogger、Sentry監視、保守ログ・収集ログ、SQLiteローカル状態、ViewModel snapshot |
| デバイス制御層（DeviceCtrl） | `Pos.DeviceCtrl` | POS 周辺機器の strategy / factory / config を管理するレイヤ |
| デバイスSDK binding | `Pos.BindingLibrary` | Epson iOS / Android SDK binding |
| UIコントロール | `Pos.ApplicationControls` | POS 向け共通コントロール / スタイル |
| アプリ共通ユーティリティ | `Pos.ApplicationUtils` | 日付 / 文字列 / ログ / 検証などのユーティリティ |
| デバイスコネクタ（Host）プロセス | `Pos.DeviceConnector` | MAUI app とは別プロセスで起動する Named Pipe host。device manager、OPOS / OCX adapter を保持する |
| 既存バイナリ | `Pos.DeviceConnector/legacyBin` | vendor / device interop DLL |

## 3. 端末アプリケーション・アーキテクチャ

### 3.1 レイヤ構成

`Pos.Applications` は以下のレイヤ構成を基本とする。
...
monitoring / local state / snapshot は `Pos.Applications` 内ではなく、共通基盤 layer の `TabletPos.Core` に配置する。device strategy / factory / config は独立 layer の `Pos.DeviceCtrl` に配置する。
...
| `Application` | `Pos.DeviceCtrl` | 可 | デバイス操作の抽象 API を呼び出す |
...
| `Pos.Applications` | `Pos.DeviceConnector` concrete device class | 不可 | デバイスコネクタ（Host）とは `Pos.DeviceCtrl` 経由で連携する |
| `Pos.DeviceCtrl` | `Pos.DeviceConnector` concrete device class | 不可 | デバイスコネクタ（Host）とは Named Pipe 契約のみで連携する |
...
ViewModel はデバイス実装、Named Pipe実装、Sentry SDK、EF Core DbContext を直接呼び出さない。デバイス操作が必要な場合は `Application` ユースケースを経由し、ユースケースが `Pos.DeviceCtrl` の strategy / service abstraction を呼び出す。
...
`Pos.Applications/Infrastructure` は、Application固有portの実装置き場である。たとえば画面遷移の `INavigationService` に対する `MauiNavigationService` を置く。これはシステム全体の infrastructure すべてを `Pos.Applications` に集約するという意味ではない。
...
Monitoring / logging / local state は `TabletPos.Core` の責務である。Device strategy / デバイスコネクタ（Host）IPC は `Pos.DeviceCtrl` の責務である。`Pos.Applications` には Sentry SDK adapter、EF Core DbContext、デバイス固有の通信実装を置かない。

SQLite local state が `Pos.Applications/Infrastructure` ではなく `TabletPos.Core` に置かれる理由は、local state、logging、monitoring をアプリ全体で共通利用する基盤として扱うためである。Application層からは `TabletPos.Core.State.IViewModelSnapshotService` や `ILocalStateSessionService` などの抽象サービスを利用し、EF Core / SQLite の実装詳細へ直接依存しない。
...
`Pos.ApplicationControls` は POS 向け UI control と style を提供する。
...
`Pos.DeviceCtrl` は、POS 周辺機器の strategy / factory / config を管理するデバイス制御 layer / solution である。

目的は、端末アプリケーションから見た device 操作 API を統一しつつ、Windows の実デバイス制御は `Pos.DeviceConnector` に集約し、iOS / Android の端末内デバイスは platform-local strategy として扱うことである。
...
Pos.Applications
 ...
 -> Pos.BindingLibrary
 -> Pos.DeviceConnector via Named Pipe
...
`Pos.DeviceCtrl` は device strategy layer である。Windows の OPOS / OCX concrete control は `Pos.DeviceConnector` に残し、iOS / Android のカメラ、BLE、Epson SDK は platform-local strategy 内に閉じ込める。platform strategy の class 名は device capability と接続方式から決定し、Application は具体 class 名へ依存しない。

### 4.3 設計要素

`Pos.DeviceCtrl` の設計要素:
...
| `Platforms/Windows` | `Pos.DeviceConnector` 連携および Windows ローカルデバイスを扱う |
...
| `Pos.BindingLibrary` | Epson iOS / Android SDK binding を提供する |

### 4.4 デバイスコネクタ連携方式

`Pos.DeviceCtrl` の strategy interface は Application から見た capability contract である。Windows / デバイスコネクタ（Host）固有の methodId、DirectIO、OPOS command 名は common interface に露出せず、Windows strategy 内の adapter mapping として扱う。

`Pos.DeviceCtrl` の Windows strategy のうち、デバイスコネクタ（Host）経由デバイスは `OposNamedPipeCommandClient` で要求と結果を変換し、`INamedPipeClient` を通じて `Pos.DeviceConnector` へコマンドを送信する。`NamedPipeClient` は `INamedPipeClient` の実装である。要求、応答、イベント、識別子、および通信既定値は `Pos.DeviceContracts` をDeviceCtrlとHostで共有する。

Scanner と Keyboard は `Pos.DeviceConnector` を経由しない。Scanner は `Pos.DeviceCtrl` 内のplatform-local strategy、Keyboard はWindowsローカルKeyboard listener strategyとして扱う。

```text
I*Strategy
 -> デバイスコネクタ（Host）経由 Opos*Strategy
 -> OposNamedPipeCommandClient
 -> Pos.DeviceContracts の要求・応答DTO
 -> INamedPipeClient / NamedPipeClient
 -> Pos.DeviceConnector.Command
...
### 4.5 プラットフォーム別ストラテジーとデバイスコネクタコマンドの対応
...
#### 4.5.1 デバイスコネクタ経由デバイス一覧

| デバイス | デバイスコネクタの`deviceId` | デバイスコネクタ実装 | デバイス制御ストラテジー |
|---|---|---|---|
...
| `Windows`向けストラテジー操作 | デバイスコネクタメッセージ | `deviceId` | `methodId` | `payload`キー | 戻り値／イベント | 備考 |
|---|---|---|---|---|---|---|
...
全 DeviceMethod request は `Sync=True`, `ProgramID=Pos.DeviceCtrl` を共通設定する。`EndDeposit` は `Success=1`、`PauseDeposit` は `Control=1`、`OpenDrawer` は `Control=1`、`DispenseChange` は `CurrentExit=0` を設定する。requestの `Handle` はpayloadではなくtop-level項目であり、現行strategyでは空文字を送信する。
...
#### 4.5.5 プラットフォーム内ストラテジー構成

Platform-local device は `Pos.DeviceConnector` の methodId へ対応させず、device capability を各OSのnative API / SDKで実現する。

| 機能 | `iOS`実装構成 | `Android`実装構成 | `Windows`ローカル構成 |
|---|---|---|---|
...
`Pos.DeviceCtrl` は config-driven strategy selection を採用する。
...
| `NamedPipeSettings` | コマンド通信用パイプ `Pos.DeviceConnector.Command`、イベント通知用パイプ `Pos.DeviceConnector.Event`、接続・応答タイムアウト、再試行回数・間隔、およびイベント再接続間隔。`DeviceManager` がコマンド通信とイベント受信の両方へ適用する。 |

`device_controller_config.json` は `Pos.DeviceCtrl` がアプリ起動時に取り込む設定ファイルである。package default は `Pos.DeviceCtrl/Resources/Raw/device_controller_config.json` に埋め込む。AppData配下のJSONを読込・検証できた場合は、設定をSQLiteへ再登録する。

AppData配下のJSONを取得または使用できない場合は、`FileSystem.AppDataDirectory/device_controller_config.db` に前回保存した設定へフォールバックする。JSONとSQLiteのどちらも使用できない場合だけpackage defaultをSQLiteへ登録する。JSON取込後を含め、実行中はSQLiteから設定モデルを復元する。

設定保存時はSQLiteへ保存し、保存に成功した場合に `DeviceManager` が新しい設定を適用する。JSONへは書き戻さない。外部JSONを更新した場合は、次回アプリ起動時に再取込する。

`host_device_config.json` は `Pos.DeviceConnector` のデバイス読込source-of-truthである。`Pos.DeviceCtrl` の `device_controller_config.json` は Application から見た有効strategy選択であり、デバイスコネクタ（Host）の device loading 設定を置き換えない。
...
`Pos.DeviceConnector` はCustomerDisplay、CashDrawer、CashChangerなどのデバイスコネクタ（Host）経由デバイスを制御するWindows device host processであり、MAUI applicationとは別プロセスとして起動する。

`Pos.DeviceConnector` は Application層でもデバイスstrategy層でもない。実行時は `Pos.DeviceCtrl` から接続される Named Pipe hostプロセスであり、Hostプロセス内で WinForms message pump、device manager、OPOS / OCX adapter を保持する。
...
`AppServer` は `Pos.DeviceConnector` process の entry point である。debug mode は `ServerAppForm` を表示し、開始 / 終了ボタンで Host を確認できる。通常運用時は画面を表示せず、`HostProcessRuntime` が `TabletHost` を起動し、WinForms message pump を保持する。
...
Windows環境のデバイスコネクタ（Host）経由デバイスでは、端末アプリケーションからの操作要求は `Pos.DeviceCtrl`、共通デバイス通信契約、名前付きパイプ、および `Pos.DeviceConnector` を経由し、最終的にデバイスコネクタ（Host）内の `IFDevice` 実装へ委譲される。対象はCustomerDisplay、CashDrawer、CashChanger、Printer、およびPaymentである。ScannerとKeyboardはこの経路を使用しない。
...
デバイスコネクタ（Host）境界では当面、要求値および応答値を辞書形式で保持する。標準化されたデバイス結果モデルは、`Pos.DeviceCtrl` 側のストラテジー結果モデルとして別途定義する。
...
| `NamedPipeCommandServer` | `Pos.DeviceConnector.Command` のrequest / responseを扱う |
| `NamedPipeEventPublisher` | `Pos.DeviceConnector.Event` へ device event をpublishする |
...
端末アプリケーションは `Pos.DeviceCtrl` を経由してデバイスコネクタ（Host）へコマンドを送信し、同期レスポンスを受け取る。DeviceCtrlとHostは `Pos.DeviceContracts` の共通要求・応答DTO、識別子、および通信既定値を使用する。Hostのイベント発行とDeviceCtrlのイベント受信も動作し、AppはHost起動後に受信を開始してウィンドウ停止・破棄時に停止する。現行ソースではEventReceivedの購読者を登録していないため、受信イベントをApplicationユースケースへ引き渡さない。

端末アプリケーションはデバイスコネクタ（Host）の `IFDevice` 実装、OPOS / OCX、Named Pipe実装を知らない。Application service は `Pos.DeviceCtrl` の strategy / service abstraction を呼び出す。

KeyboardとScannerはデバイスコネクタ（Host）経由デバイスコマンドの対象外であり、`Pos.DeviceCtrl` 内のローカルstrategyまたはlistenerからApplicationユースケースへ通知する。デバイスコネクタ（Host）にScanner実装とScanner専用pipeは持たせない。PrinterとPaymentはデバイスコネクタ（Host）の対象である。

### 6.2 パイプ一覧

| パイプ | 方向 | 用途 |
|---|---|---|
| `Pos.DeviceConnector.Command` | DeviceCtrl client -> Host process -> DeviceCtrl client | コマンド要求 / 同期レスポンス |
| `Pos.DeviceConnector.Event` | Host process -> DeviceCtrl listener | Hostがイベントを発行し、Appライフサイクルで開始されたNamedPipeEventReceiverが受信する。現行ソースではEventReceivedの購読者を登録していない。 |
...
デバイス側が `ReplyDevice(...)` を呼び出すと、デバイスコネクタ（Host）は `Pos.DeviceConnector.Event` へ名前付きパイプイベントを発行する。AppはHost起動後にNamedPipeEventReceiverを開始し、受信した1行JSONをNamedPipeDeviceEventへ変換してEventReceivedを発火する。現行ソースではEventReceivedの購読者を登録していないため、Applicationユースケースまでの引渡しは成立しない。
...
`Pos.DeviceCtrl` からデバイスコネクタ（Host）を呼び出す場合は、以下を原則とする。

```text
Presentation/ViewModels
 -> Application/<Module>/Commands or Queries
 -> Pos.DeviceCtrl/Interfaces/I*Strategy
 -> Windows Opos*Strategy（デバイスコネクタ（Host）経由）
 -> NamedPipeClient
 -> Pos.DeviceConnector.Command
```

ViewModel からNamed Pipe clientを直接作らない。Named Pipe timeoutとcommand / responseの組み立ては `Pos.DeviceCtrl` の通信実装に閉じ込める。retry / reconnectは将来実装する場合も同じ境界に配置し、現行実装済みとは扱わない。

## 7. 共通基盤方式

### 7.1 DI

DI 登録は `Pos.Applications/Composition/DependencyInjection.cs`、`TabletPos.Core/ServiceCollectionExtensions.cs`、`Pos.DeviceCtrl/ServiceCollectionExtensions.cs` に分担する。

`TabletPos.Core` は `AddTabletPosCore(appDataPath)` で共通基盤を登録する。
`Pos.DeviceCtrl` は `AddPosDeviceCtrl()` で device strategy / factory / named pipe client を登録する。
...
`Pos.Applications` 側の DI ではデバイスユースケース / facade を登録し、strategy factory / named pipe communication の詳細は `Pos.DeviceCtrl` 側の composition extension に閉じ込める。
...
監視は `TabletPos.Core.Monitoring.IMonitoringService` を通じて行う。Sentry はクラッシュ / エラーEventの収集およびエラー発生前のBreadcrumb参照に使用する。Sentry SDK の初期化は `Pos.Applications/MauiProgram.cs` で行い、設定値と送信前マスキングは `TabletPos.Core.Monitoring` が提供する。
...
| イベント | レベル | カテゴリ | `Sentry`での扱い | 記録内容 |
|---|---|---|---|---|
...
| 対象 | 出力先 | 用途 |
|---|---|---|
| `Pos.Applications` | `TabletPos.Core.Monitoring` 経由の Sentry Breadcrumb / Event | 画面遷移、ライフサイクル、エラー、ローカル状態操作の追跡 |
| `Pos.Applications` | `TabletPos.Core.Logging.IAppLogger` 経由のローカルログ | アプリ側の保守ログ・収集ログ |
| `Pos.DeviceCtrl` | `TabletPos.Core.Logging.IAppLogger` 経由のローカルログ | strategyコマンド、デバイスコネクタ（Host）command、platform-local deviceイベントの追跡 |
| `Pos.DeviceConnector` | 既存 `KsLogControls` ローカルログ | デバイスアダプタ、OPOS / OCX、Hostプロセス内部処理の調査 |

Named Pipe response はデバイス応答の通信経路であり、ローカルログの代替ではない。デバイスコネクタ（Host）のデバイス固有調査にはデバイスコネクタローカルログを保持する。

ログレベルごとの扱いは以下とする。

| レベル | ローカル保存 | `Sentry` | 用途 |
|---|---|---|---|
...
#### 7.4.4 端末アプリ／デバイス制御層ログ出力ケース

| ケース | レベル | カテゴリ | 出力先 | 記録内容 |
|---|---|---|---|---|
...
sources/TabletPosBoilerplate/
 ...
 Pos.DeviceCtrl/
 Pos.BindingLibrary/
 Pos.ApplicationControls/
 Pos.ApplicationUtils/
 Pos.DeviceConnector/
 src/
 AppServer/
 AppStopServer/
...
| モジュール | 責務 |
|---|---|
| `Pos.Applications` | MAUIアプリシェル、ViewModel、画面遷移、Applicationユースケース |
| `TabletPos.Core` | IAppLogger、Sentry monitoring、保守ログ・収集ログ、SQLite local state、ViewModel snapshot |
| `Pos.DeviceContracts` | DeviceCtrl、Host、AppStopServer、およびintegration runnerで共有する名前付きパイプ要求・応答・イベントDTO、識別子、通信既定値、およびOPOSプリンター既定値 |
| `Pos.DeviceCtrl` | POS周辺機器のstrategy / factory / configレイヤ。共通デバイス通信契約を用いてHostへ接続し、platform-local deviceも扱う |
| `Pos.BindingLibrary` | Epson iOS / Android SDK binding |
| `Pos.ApplicationControls` | 再利用可能な POS UIコントロールとスタイル |
| `Pos.ApplicationUtils` | アプリ共通ユーティリティ関数 |
...
`Pos.Applications` は MAUI application として build / package する。Windows 端末ではunpackaged modeを前提とする。
...
`Pos.DeviceCtrl` は端末アプリケーションと同一端末上で動作するライブラリとして配備する。Windows 環境では `Pos.DeviceConnector` の Named Pipe に接続できる設定を持ち、iOS / Android 環境では platform-local device の接続設定を持つ。

必要な実行時設定:

- 有効デバイスstrategy設定
- command pipe名: `Pos.DeviceConnector.Command`
- timeout設定
- camera scanner 設定
- BLE scanner 設定
- Epson SDK printer / display 設定

イベント通知用パイプ名は `Pos.DeviceConnector.Event` とする。DeviceManagerが同じNamedPipeSettingsをNamedPipeEventReceiverへ適用し、AppがHost起動後に受信を開始する。受信失敗時はDeviceCommandDefaults.EventReconnectIntervalMillisecondsまたは設定値を使用して再接続する。現行ソースではEventReceivedの購読者を登録していない。

### 9.3 デバイスコネクタ

`Pos.DeviceConnector` は Windows Forms hostプロセスとして配備する。
...
| `device_controller_config.json` | `Pos.DeviceCtrl/Resources/Raw/device_controller_config.json` / `FileSystem.AppDataDirectory/device_controller_config.json` | デバイス制御層（DeviceCtrl）側の有効strategy選択、接続設定、Named Pipe 設定 |
| `device_controller_config.db` | `FileSystem.AppDataDirectory/device_controller_config.db` | 起動時JSON取込後とJSON取込不可時のフォールバックで使用し、実行中に読み込み・保存するデバイス制御設定。テーブル構成はDB-DEVICE-01で定義する |
...
```powershell
dotnet build sources/pos-integration/Pos.Applications/Pos.Applications.csproj -f net10.0-windows10.0.19041.0 -v:m
```

Hostビルド:

```powershell
dotnet build sources/pos-integration/Pos.DeviceConnector/src/DeviceConnector/DeviceConnector/TabletHost.csproj -p:EnableWindowsTargeting=true /m:1
```

DeviceCtrlビルド:

```powershell
dotnet build sources/pos-integration/Pos.DeviceCtrl/Pos.DeviceCtrl.csproj -f net10.0-windows10.0.19041.0 -v:m
```

### 10.2 単体テスト方針

単体テストでは、DeviceCtrlのHost mapping、Scanner／Keyboardのローカル制御、Sentry／local state、Hostのコマンド経路、および`host_device_config.json`とclass IDの対応を確認する。現行のcheckoutにはテストprojectが含まれていないため、実行可能な自動テストの数と結果は記載しない。
...
| 種別 | 主な対象 |
|---|---|
| Application architecture | `Pos.Applications`、画面遷移、Applicationユースケース |
| Core architecture | `TabletPos.Core`、IAppLogger、保守ログ・収集ログ、Sentry monitoring、SQLite local state |
| Device control architecture | `Pos.DeviceCtrl`、`Pos.BindingLibrary`、デバイスstrategy、factory、config設計 |
| Device host architecture | `Pos.DeviceConnector`、Named Pipe、デバイス読込、OCX threading |
| Device-specific knowledge | Scanner DENSO、CustomerDisplay、CashDrawer、CashChanger RT300、OPOS / OCX |
| Reference source | `sources/pos-integration/Pos.Applications`、`sources/TabletPosBoilerplate/TabletPos.Core`、`sources/pos-integration/Pos.DeviceCtrl`、`sources/pos-integration/Pos.BindingLibrary`、`sources/pos-integration/Pos.DeviceConnector` |
