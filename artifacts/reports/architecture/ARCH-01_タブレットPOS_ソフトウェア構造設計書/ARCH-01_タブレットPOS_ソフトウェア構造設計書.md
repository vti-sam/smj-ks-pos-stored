...
| :--------- | :---- | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :----- | :----- |
| 改訂日     | 版数  | 内容                                                                                                                                                                     | 改訂者 | 承認者 |
| 2026/06/04 | 1.0.0 | タブレットPOS の構成に合わせ、端末アプリケーション、デバイス制御レイヤ、デバイスホスト、Named Pipe 連携、ローカル状態保持、監視方式を中心とした構造設計として改訂 | VTI    | -      |
...
| 2026/06/18 | 1.0.0 | CustomerDisplay 契約、Scanner / Keyboard のローカル制御、Host command mapping、イベント未実装範囲、テスト実績を現行ソースに合わせて更新 | VTI    | -      |
|            |       |                                                                                                                                                                          |        |        |
...
- [4. デバイス制御レイヤ・アーキテクチャ](#4-デバイス制御レイヤアーキテクチャ)
  - [4.1 位置づけ](#41-位置づけ)
  - [4.2 レイヤ構成](#42-レイヤ構成)
  - [4.3 設計要素](#43-設計要素)
  - [4.4 Host 連携方式](#44-host-連携方式)
  - [4.5 Windows strategy と Host command の対応](#45-windows-strategy-と-host-command-の対応)
    - [4.5.1 Host-backed device 一覧](#451-host-backed-device-一覧)
  ...
  - [4.6 設定方針](#46-設定方針)
  - [4.7 実装段階](#47-実装段階)
- [5. デバイスホスト・アーキテクチャ](#5-デバイスホストアーキテクチャ)
  - [5.1 Host の役割](#51-host-の役割)
  - [5.2 起動フロー](#52-起動フロー)
  - [5.3 停止フロー](#53-停止フロー)
  - [5.4 Host device config](#54-host-device-config)
...
- [6. 端末アプリケーション・ホスト連携方式](#6-端末アプリケーションホスト連携方式)
  - [6.1 基本方針](#61-基本方針)
...
- [9. 配備・設定方式](#9-配備設定方式)
  ...
  - [9.2 デバイス制御レイヤ](#92-デバイス制御レイヤ)
  - [9.3 デバイスホスト](#93-デバイスホスト)
...
- Windows の CustomerDisplay、CashDrawer、CashChanger は `TabetPos.Host` を経由して制御する。Scanner と Keyboard は Host を経由せず `TabetPos.DeviceCtrl` 内でローカル制御する。
- `TabetPos.Host` は OPOS / OCX / ActiveX を利用する Host-backed デバイスを扱う Windows Forms プロセスである。
- Windows Printer は別 Host への統合を予定しているが、現時点では未統合であり、既存 strategy を維持する。
- 新しい Application-Host 通信経路として Named Pipe を使用する。
- WindowMessage は既存互換のために保持する。
...
タブレットPOS は、端末アプリケーションとデバイスホストを分離する構成を基本とする。
...
- Host-backed デバイスの open / claim / enable / release / close は Host 側に閉じ込める。
- 端末アプリケーションから Host-backed デバイスの OPOS / OCX / ActiveX を直接呼び出さない。
...
- `TabetPos.DeviceCtrl` は `TabetPos.Core.Logging.IAppLogger` を共通ログポートとして利用し、デバイス制御レイヤ内に独自 logger 基盤を持たない。
...
- Host は WindowMessage 互換を保持しつつ、Named Pipe command経路とHost側Event publisherを提供する。DeviceCtrl側Event listenerは未実装である。
- レガシーデバイスは Host 側の `host_device_config.json` により動的ロードする。
...
| Host UI / message pump | Windows Forms | OPOS / OCX host form |
| Device protocol | WindowMessage | レガシー互換のデバイスコマンド経路 |
...
| `TabetPos.DeviceCtrl` | .NET class library / MAUI互換デバイス制御レイヤ |
| `TabetPos.BindingLibrary` | iOS / Android Epson SDK binding |
| `TabetPos.ApplicationControls` | MAUIコントロールプロジェクト |
| `TabetPos.Host` | Windows Forms / 外部Named PipeデバイスHostプロセス |

Windows POS 端末での実行を主対象とする。OPOS / OCX / ActiveX を扱う Host は Windows 前提である。
...
| デバイス制御レイヤ | `TabetPos.DeviceCtrl` | POS 周辺機器の strategy / factory / config を管理するレイヤ |
...
| デバイスHostプロセス | `TabetPos.Host` | MAUI app とは別プロセスで起動する Named Pipe host。WindowMessage、device manager |
| Legacy binaries | `TabetPos.Host/legacyBin` | WindowMessage / vendor / device interop DLL |
...
| `TabetPos.Applications` | `TabetPos.Host` concrete device class | 不可 | Host とは `TabetPos.DeviceCtrl` 経由で連携する |
| `TabetPos.DeviceCtrl` | `TabetPos.Host` concrete device class | 不可 | Host とは Named Pipe 契約のみで連携する |
...
Monitoring / logging / local state は `TabetPos.Core` の責務である。Device strategy / Host IPC は `TabetPos.DeviceCtrl` の責務である。`TabetPos.Applications` には Sentry SDK adapter、EF Core DbContext、デバイス固有の通信実装を置かない。
...
| `INamedPipeClient` | Windows strategy から Host named pipe へ command を送信する通信ポート |
| `Interfaces/*Strategy` | Application から見た device capability contract を定義する。Windows / Host 固有 command 名を共通 interface としない |
...
`TabetPos.DeviceCtrl` の strategy interface は Application から見た capability contract である。Windows / Host 固有の methodId、DirectIO、OPOS command 名は common interface に露出せず、Windows strategy 内の adapter mapping として扱う。

`TabetPos.DeviceCtrl` の Windows strategy のうち、Host-backed device は `INamedPipeClient` を通じて `TabetPos.Host` へ command を送信する。`NamedPipeClient` は `INamedPipeClient` の実装であり、strategy からは DI で注入して使用する。
...
- Host の Event publisher は実装済みだが、`TabetPos.DeviceCtrl` 側の Event listener は未実装である。

### 4.5 Windows strategy と Host command の対応

本節の対応表は Windows Host-backed strategy の adapter mapping を示す。iOS / Android strategy は同じdevice capabilityをCamera / BLE / Epson SDKで実現するが、Host methodIdへの準拠は要求しない。

#### 4.5.1 Host-backed device 一覧

| Device | Host deviceId | Host implementation | DeviceCtrl strategy |
|---|---|---|---|
| Customer display | `CustomerDisplay` | `LineDisplayBySharp`（物理実装のlegacy名称） | `OposCustomerDisplayStrategy` |
...
`SerialCashChangerStrategy` はローカル接続であり、Host-backed mapping には含めない。Printer は別 Host への統合予定で、現時点の mapping には含めない。
...
Host は `LinDsp` / `LinDspTelop` も処理できるが、現行の `ICustomerDisplayStrategy` からは公開していない。`SetDescriptor`, `CreateWindow`, `DestroyWindow`, `RefreshWindow`, `DirectIo` は interface に存在するが、Windows strategy では `NotSupportedException` とする。
...
`CheckStatus` は現行 Host mapping では未対応である。
...
| Printer | Epson SDK strategy | Epson SDK strategy | 現行strategyを維持する。別 Host への統合は将来対応 |
| Customer display | Epson SDK customer display strategy | Epson SDK customer display strategy | Host-backed CustomerDisplay adapter mapping に従う |
| Drawer | Epson printer pulse または drawer strategy | - | Host-backed CashDrawer adapter mapping に従う |
...
`host_device_config.json` は `TabetPos.Host` のデバイス読込source-of-truthである。`TabetPos.DeviceCtrl` の `device_controller_config.json` は Application から見た有効strategy選択であり、Host の device loading 設定を置き換えない。
...
| Windows Host-backed device | CustomerDisplay、CashDrawer、CashChanger | Named Pipe command、Host応答、result mapping |
| Windows Printer | 既存strategyを維持。別 Host への統合は将来対応 | 現時点では Host command mapping の対象外 |
...
Android は platform-local strategy の実装枠を持つが、Windows と同じ Host command対応を要求しない。

## 5. デバイスホスト・アーキテクチャ

### 5.1 Host の役割

`TabetPos.Host` はCustomerDisplay、CashDrawer、CashChangerなどのHost-backed周辺機器を制御するWindows device host processであり、MAUI applicationとは別プロセスとして起動する。

`TabetPos.Host` は Application層でもデバイスstrategy層でもない。実行時は `TabetPos.DeviceCtrl` から接続される Named Pipe hostプロセスであり、Hostプロセス内で WinForms message pump、WindowMessage互換、device manager、OPOS / OCX adapter を保持する。

主な責務:

- WindowMessage 互換service host の起動・停止。
- Named Pipeコマンド / イベントの起動・停止。
- `host_device_config.json` に基づくデバイスクラスの動的ロード。
- `IFDevice` 実装へのコマンドdispatch。
- OPOS / OCX / ActiveX のライフサイクル管理。
- レガシーデバイス結果dictionaryの保持。
...
AppServer
  ...
  -> KsHost.StartHost()
     ...
     -> DeviceHostTransport.Start(commandHandler)
        -> NamedPipeDeviceHostAdapter.Start()
        -> WindowMessageDeviceHostAdapter.Start()
     -> IFSettingDevice を生成
...
`AppServer` は `TabetPos.Host` process の entry point である。debug mode は `ServerAppForm` を表示し、開始 / 終了ボタンで Host を制御する。non-debug mode では `HostProcessRuntime` が `KsHost` を起動し、`IsEndOrder` まで WinForms message pump を保持する。
...
KsHost.StopHost()
  -> DeviceHostTransport.Stop()
     -> NamedPipeDeviceHostAdapter.Dispose()
     -> WindowMessageDeviceHostAdapter.Stop()
  -> KsDeviceManager.StopDeviceManager()
     -> foreach device StopDevice()
     -> loaded device snapshot をclear
```

Host 終了時は WinForms message pump、device form、OCX の状態が関係するため、単純な process kill ではなく Host の停止処理を通す。

### 5.4 Host device config

Device loading は `host_device_config.json` で制御する。汎用的な設定名は使用せず、Host 側の設定であることをファイル名で明示する。
...
| Key | 内容 |
|---|---|
| `devices[].id` | Host 内で扱う論理device ID。Customer display は `CustomerDisplay` を使用する |
| `devices[].name` | runtime device display name |
| `devices[].visible` | device form visibility |
| `devices[].classId` | `KsClassFactory` で生成する runtime class ID |
| `devices[].productName` | device product name |

`LineDisplay1` は `KsClassID` / `KsClassFactory` が使用する物理実装側のlegacy class IDである。Application / DeviceCtrl / Host間の論理IDは `CustomerDisplay` に統一するが、このclass IDは変更しない。
...
`IFDevice` 境界は、Host プロセス内のデバイス制御実装を統一的に呼び出すためのインターフェース境界である。各デバイス実装は `IFDevice` を実装し、Host はデバイスID、利用開始、利用終了、メソッド実行を同一の契約で扱う。

Windows 環境のHost-backedデバイスでは、端末アプリケーションからの操作要求は `TabetPos.DeviceCtrl`、Named Pipe、`TabetPos.Host` を経由し、最終的に Host 内の `IFDevice` 実装へ委譲される。Scanner / Keyboard はこの経路を使用しない。Printer は別 Host への統合予定であり、現時点ではこの経路へ未統合である。

| メンバー | 概要 |
|---|---|
| `KsDeviceId` | デバイスID |
| `KeepAliveDateTime` | デバイス応答の最終生存確認日時 |
| `StartDevice(KsDeviceId)` | デバイス起動 |
| `StopDevice()` | デバイス停止 |
| `DeviceMethod(KsDeviceMethodID, Dictionary<string,string>, ref Dictionary<string,string>)` | デバイスメソッド実行 |
...
Host 境界では当面、要求値および応答値を辞書形式で保持する。標準化されたデバイス結果モデルは、`TabetPos.DeviceCtrl` 側のストラテジー結果モデルとして別途定義する。

### 5.6 DeviceCommandRouter

Host 内部では、transport と device command handling を分離する。Named Pipe と WindowMessage は入力形式が異なるが、どちらも `DeviceCommand` に正規化して `DeviceCommandHandler` へ渡す。

主な内部要素:

| 要素 | 役割 |
|---|---|
| `DeviceCommand` / `DeviceCommandResult` | transport に依存しない Host command / result model |
| `DeviceCommandHandler` | `DeviceUse`, `DeviceUnUse`, `DeviceUnUseComplete`, `DeviceMethod`, `Kill`, `ReStart` の共通処理 |
| `LegacyMessageParser` | WindowMessage および `legacyMessage` の tab-delimited message をdictionaryへ変換 |
| `NamedPipeCommandMapper` | Named Pipe request / response と `DeviceCommand` / `DeviceCommandResult` の相互変換 |
| `DeviceHostTransport` | WindowMessage adapter と Named Pipe adapter の起動・停止・reply publish を集約 |
| `WindowMessageDeviceHostAdapter` | レガシーWindowMessage受信、応答、`ReplyMessage` retry stock を保持 |
| `NamedPipeDeviceHostAdapter` | `TabetPos.Host.Command` のrequest / responseと、`TabetPos.Host.Event` のpublishを扱う |
...
`KsDeviceManager` は loaded device の snapshot を公開し、device lookup は `FindDevice(KsDeviceId)` 経由で行う。Host command handler はこの registry interface を通じて `IFDevice` を取得し、mutable list へ直接依存しない。

`KsProcessInfo` は `(deviceId, handle)` をkeyにした thread-safe store として保持する。CashChanger などのlegacy deviceがstatic facadeを参照するため、`ProcessInfoes` などの既存 API は当面互換として残す。
...
|---|---|---|
| CustomerDisplay | `LineDisplayBySharp`（物理実装のlegacy名称） | `AxInterop.LINEDISPLAYLib.dll`, `Interop.LINEDISPLAYLib.dll` |
...
- 主なコマンドは `OpenDrawer`。
- デバイス境界は比較的小さいが、OPOS / OCX lifecycle は Host 側に保持する。

#### CashChanger RT300

- `AsyncMode`, `FreezeEvents`, `DataEventEnabled` が重要。
- `BeginDeposit` / `EndDeposit` など入金状態はhandleと結びつく。
- レガシーファイル連携として `TURIREQ.DAT`, `TURIANS.DAT`, `TURIREQ_OK.DAT`, `TURIANS_OK.DAT`, `TURIANS_NG.DAT` を扱う経路が残る。
- 特殊な結果コードやvendor固有ガイダンスはadapter側に閉じ込める。

## 6. 端末アプリケーション・ホスト連携方式

### 6.1 基本方針

端末アプリケーションは `TabetPos.DeviceCtrl` を経由して Host-backed デバイスコマンドを送信し、同期レスポンスを受け取る。このcommand経路は実装済みである。Host側のEvent publisherも実装済みだが、`TabetPos.DeviceCtrl` 側のEvent listenerは未実装のため、Host callbackのApplicationユースケースへの通知は今後の対応となる。

端末アプリケーションは Host の `IFDevice` 実装、OPOS / OCX、Named Pipe実装を知らない。Application service は `TabetPos.DeviceCtrl` の strategy / service abstraction を呼び出す。

Keyboard と Scanner は Host-backed デバイスコマンドの対象外であり、`TabetPos.DeviceCtrl` 内のローカルstrategy / listenerからApplicationユースケースへ通知する。HostにScanner実装とScanner専用pipeは持たせない。Printerは別Hostへの統合を予定しているが、現時点では未統合である。
...
| `TabetPos.Host.Event` | Host process -> DeviceCtrl listener | Host側publisherは実装済み。DeviceCtrl側listenerは未実装 |
...
`LegacyMessage` が指定された場合、Host はタブ区切りのレガシーメッセージとしてparseする。
...
Host は device return dictionary に `ResultCode` と `ReturnValue` が無い場合、`returnValue` から補完する。
...
デバイス側が `ReplyDevice(...)` を呼び出すと、Host は WindowMessage応答に加えて `TabetPos.Host.Event` へNamed Pipeイベントをpublishする。現時点ではDeviceCtrl側listenerが未実装のため、イベントのend-to-end受信は成立していない。
...
`TabetPos.DeviceCtrl` から Host を呼び出す場合は、以下を原則とする。

```text
Presentation/ViewModels
  -> Application/<Module>/Commands or Queries
    -> TabetPos.DeviceCtrl/Interfaces/I*Strategy
      -> TabetPos.DeviceCtrl/Platforms/Windows/Strategies/Host-backed Opos*Strategy
        -> NamedPipeClient
          -> TabetPos.Host.Command
```
...
| Error | 業務が正常完了できないエラー、重大エラー、またはクラッシュ | 画面遷移失敗、Host commandタイムアウト、デバイスエラー、未処理例外 |
...
|---|---|---|
| DSN | `Sentry:Dsn` | `KSPOS_SENTRY_DSN` |
| Environment | `Sentry:Environment` | `KSPOS_SENTRY_ENVIRONMENT` |
| Enable / disable | `Sentry:EnableSentry` | `KSPOS_SENTRY_ENABLED` |
...
| `TabetPos.DeviceCtrl` | `TabetPos.Core.Logging.IAppLogger` 経由のローカルログ | strategyコマンド、Host command、platform-local deviceイベントの追跡 |
| `TabetPos.Host` | legacy `KsLogControls` ローカルログ | デバイスアダプタ、OPOS / OCX、Hostプロセス内部処理の調査 |

Named Pipe response はデバイス応答の通信経路であり、ローカルログの代替ではない。Host のデバイス固有調査には Hostローカルログを保持する。
...
| Host command送信 | Info | DeviceCtrl | Sentry Breadcrumb、ローカルログ | requestId、deviceId、methodId |
| Host command応答 | Info | DeviceCtrl | Sentry Breadcrumb、ローカルログ | requestId、deviceId、methodId、resultCode、処理時間(ms) |
| Host commandタイムアウト | Error | DeviceCtrl | Sentry Event、ローカルログ | requestId、deviceId、methodId、timeout |
...
- strategy は Host command契約に従って device 操作を送信する。
- Host接続タイムアウト / payload不足はデバイス制御レイヤのエラーとしてログ / 監視へ渡す。
...
| `TabetPos.DeviceCtrl` | POS 周辺機器の strategy / factory / config レイヤ。KsHost Named Pipe契約と platform-local device を扱う |
...
| `AppStopServer` | レガシーWindowMessage終了要求送信 |
| `KsHost/DeviceHost` | Hostプロセス中核、command core、WindowMessage adapter、Named Pipe adapter、コマンドrouting |
| `KsDeviceManager` | デバイス読込、ライフサイクル、keep-alive監視、loaded device snapshot / lookup |
| `KsDeviceDefinition` | デバイスID、メソッドID、定数、インターフェース |
| `KsDevice/*` | 実OPOS / OCXデバイスadapter |
| `legacyBin` | interop / vendor / レガシー共通DLL |
...
| DeviceCtrl -> Host | `NamedPipeClient` と Host named pipe 経由で接続する |
...
| 設定 | 所在 | 用途 |
|---|---|---|
| `host_device_config.json` | `AppServer/Resources/host_device_config.json` | Host 側のデバイス読込、runtime class ID、device-specific parameter |
| `device_controller_config.json` | `TabetPos.Applications/Resources/Raw/device_controller_config.json` / `FileSystem.AppDataDirectory/device_controller_config.json` | DeviceCtrl 側の有効strategy選択、接続設定、Named Pipe 設定 |
...
| Host legacy config | `AppServer/app.config`, `AppStopServer/app.config` | レガシーprobing / ログパス |
...
```powershell
dotnet build sources/POS 開発用ベースプロジェクト/TabetPos.Core/TabetPos.Core.csproj -f net10.0-windows10.0.19041.0 -v:m
```

Applicationビルド:

```powershell
dotnet build sources/POS 開発用ベースプロジェクト/TabetPos.Applications/TabetPos.Applications.csproj -f net10.0-windows10.0.19041.0 -v:m
```

Hostビルド:

```powershell
dotnet build sources/POS 開発用ベースプロジェクト/TabetPos.Host/TabetPos.Host.slnx -v:m
```

DeviceCtrlビルド:

```powershell
dotnet build sources/POS 開発用ベースプロジェクト/TabetPos.DeviceCtrl/TabetPos.DeviceCtrl.csproj -f net10.0-windows10.0.19041.0 -v:m
```
...
| `TabetPos.Host/tests/KsDeviceSettingBase.Tests` | Host device config、class ID、OPOS runtime connectivityを検証。実機Openは `TABETPOS_OPOS_RUN_HARDWARE_OPEN_TESTS=1` 指定時のみ実行。2026/06/18のローカル確認ではconfig系2件成功、実機Open 1件skip、COM生成1件はregistryがrename前のHost runtime pathを参照しているため失敗 |
...
| DeviceCtrl strategy | strategy method が Host command へ正しく変換される |
...
| WindowMessage互換 | レガシークライアントの `ReplyMessage` retry が保持される |
...
- 論理device IDは `CustomerDisplay` を使用し、物理実装のlegacy名称 `LineDisplay1`, `LineDisplayBySharp`, vendor interop名称は変更しない。
- `ResultCode`, `ResultCodeExtended`, `ErrorMessage`, `ReturnValue` のレガシーkeyを削除しない。
- OCX form / timer / message pump boundary を削除しない。
- Hostローカルログを Named Pipeレスポンスで置き換えない。
- DeviceCtrl の Host command mapping を Application UI へ露出しない。
...
| Device host architecture | `TabetPos.Host`、Named Pipe、WindowMessage、デバイス読込、OCX threading |
| Device-specific knowledge | Scanner DENSO、CustomerDisplay、CashDrawer、CashChanger RT300、OPOS / OCX |
| Reference source | `sources/POS 開発用ベースプロジェクト/TabetPos.Applications`、`sources/POS 開発用ベースプロジェクト/TabetPos.Core`、`sources/POS 開発用ベースプロジェクト/TabetPos.DeviceCtrl`、`sources/POS 開発用ベースプロジェクト/TabetPos.BindingLibrary`、`sources/POS 開発用ベースプロジェクト/TabetPos.Host` |
